...
String DateNewsViewTime[] = {"9","18","27","42","54"}; // aktuelles Datum, Greetings/Feiertage und News (Minute) 
...
// #####################################################################################
  ...
  if (!IsBuzzer) {
  if (SerialMonitor) {Serial.println("Summer (Buzzer) nicht aktiv");}} else {
  if (SerialMonitor) {Serial.println("Summer (Buzzer) ist aktiv");}}

  DateString = ExpandedDate(); // Datum mit Kalenderwoche berechnen
