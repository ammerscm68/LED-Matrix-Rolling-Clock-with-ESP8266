...
if (littleFSInit && ResetNow) {ResetClock();}} else {PleaseWait();}} // Settings to Standard - WLAN Access Data will be deleted 
...
HTMLString += F("<a><br /></a>");
if (!MEP && !await) {
HTMLString += "<h7>Runtime of the Clock: &nbsp;&nbsp;"+rtime+"</h7>";} else {
HTMLString += F("<h7>Runtime of the Clock: &nbsp;&nbsp; Please wait ...</h7>");}
...
HTMLString = F("<!DOCTYPE html> <html lang='en-US'>\
  ...
  <style>\
   ...
   h3 {font-size: 28px; margin-top: 0px; margin-bottom: 10px; margin-left: 10px;}\
    ...
    <a><br /></a>\
     <h3>LED-Matrix-Clock Reset !</h3>\
     <h2>(The WLAN Access Data will be deleted)</h2>\
     <a><br /></a>\
     <h2>Now Reset the LED Matrix Clock ?</h2>\
     <a><br /></a>\
     <a href =\"/goreset\"><button class=\"button\" style=\"height: 48px; width: 110px; font-size: 22px; color: red\">Yes</button></a>\
  ...
  <style>\
   ...
   h3 {font-size: 28px; margin-bottom: 10px; margin-left: 10px;}\
    ...
    <h1>LED-Matrix-Clock</h1>\
     ...
     <h2>(Restarting the Clock resolves the Problem.)</h2>\
     <a><br /></a>\
     <h2>Restart LED-Matrix-Clock now?</h2>\
     <a><br /></a>\
     <a href =\"/reboot\"><button class=\"button\" style=\"height: 48px; width: 110px; font-size: 22px; color: red\">YES</button></a>\
     <a href =\"/\"><button class=\"button\" style=\"margin-left: 80px;height: 48px; width: 110px; font-size: 22px;\">NO</button></a>\
  </body>\
    </html>"); 
OneTimeCall = true;        
...
await = false;
if (SerialMonitor) {DEBUG_PRINTLN(F("Buzzer Hoursound switched on"));}} else {PleaseWait();}
