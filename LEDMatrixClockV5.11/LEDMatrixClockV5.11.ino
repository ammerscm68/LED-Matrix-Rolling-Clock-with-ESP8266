...
if (littleFSInit == true)
{
  // ***First Start***
     ...
     //LittleFS.format();
     //SetupSave("firstinit.txt", "Init"); // Init-Datei anlegen
     
  InitFile = LittleFS.exists("/firstinit.txt");
  ...
  if (OWMF == "Off") {WeatherFunctions = false;}
} else
    {
  ...
  //if (InitFile){wifiManager.resetSettings();} // alle Einstellungen zurücksetzen
  ...
  if (!InitFile) {
    // Serial.println("");
    // Serial.println("InitFile not exist"); // ab dem zweiten Start der Uhr - SetupSave("firstinit.txt", "Init") vorher deaktivieren !!! 
    // Serial.println("");
    } else 
    {
    Serial.println("");
  ...
  // INIT-Datei wieder löschen wenn vorhanden
  InitFile = LittleFS.exists("/firstinit.txt");
  if (InitFile){LittleFS.remove("/firstinit.txt");}
