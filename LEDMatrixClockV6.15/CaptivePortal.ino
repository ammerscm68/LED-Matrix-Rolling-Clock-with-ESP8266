...
if (server.method() == HTTP_POST) {
    ...
    DisplayOnTime = ""; // Funktion deaktiviert
    DisplayOffTime = ""; // Funktion deaktiviert

    // Trim Strings
    TempSSID.trim(); password.trim(); weatherKey.trim(); cityID.trim();
    newsKey.trim(); InTempOffset.trim(); InHumOffset.trim();
    ...
    HTMLString += "</body>";
    HTMLString += "</html>";
    server.send(200, "text/html", HTMLString);}  // zurück zur Startseite   
  } else {
    ...
    server.send(200, "text/html", ""); // Captive Portal 
