...
// pinMode(A0, INPUT); 
  // Taster initialisieren
   ...
   // DHT 11 Sensor
   // dht.setup(5, DHTesp::DHT11); // Connect DHT sensor to GPIO 5 (D1) - Temperatur- und Luftfeuchtesensor für Innen
   // DHT 22 Sensor
   dht.setup(5, DHTesp::DHT22); // Connect DHT sensor to GPIO 5 (D1) - Temperatur- und Luftfeuchtesensor für Innen
// #################################################################################################################  
...
// contrast(); //AutoBrightness
 ...
 if (!await) {MatrixButtonOnOff();} 
  ...
  if (StartFail) {if (SerialMonitor) {Serial.println("Uhr wird neu gestartet...");} ESP.restart();} // Uhr neu starten bei WLAN Problemen
  if (!await) {
  // ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  // StartSound
  if (Display) {StartBuzzerBeep();} // Sound beim Start  
  await = false;}}
...
if (InTempValue != "-" || WeatherFunctions || NewsAPIFunction) { 
