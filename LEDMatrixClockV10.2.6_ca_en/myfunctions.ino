...
if (SerialMonitor) {Serial.println(F("Load Clock Adjustments ..."));}
  ...
  if (NTPSN != "") {ntpServerName = NTPSN.c_str();} else {ntpServerName = "ie.pool.ntp.org";} // Default
 ...
 Serial.println(F(""));}
  ...
  LoadClockStatus(); // Load the last Status of the Clock
    ...
    MCTZ = "Coral Harbour, Pond Inlet, Atikokan, Toronto, Montreal, Ottawa"; // Default Time Zone 
...
MyWaitLoop(500); //wait     
sendCmdAll(CMD_SHUTDOWN,0); // Display OFF
if (SerialMonitor) {Serial.println("Matrix switched off via Button at "+ FormatedDateTimeEN(false));}} else { 
APStart += 1; // inc   
if (SerialMonitor) {Serial.println("Reset Counter: "+ String(APStart));}}
} else { 
if (!Start) { 
    MyWaitLoop(500); // wait     
    ...
    // ShortBuzzerBeep();
    if (SerialMonitor) {Serial.println("Matrix switched on via Button at "+ FormatedDateTimeEN(false));}}
    APStart += 1; // inc 
    if (SerialMonitor && Start) {Serial.println("Reset Counter: "+ String(APStart));} 
...
clr(); // Display Clear
refreshAll(); 
if (Start) {if (SerialMonitor) {Serial.println(F("WLAN-Connection failed ..."));}} else {
if (SerialMonitor) {Serial.println(F("WLAN-Connection disconnected ..."));}}
} else {
