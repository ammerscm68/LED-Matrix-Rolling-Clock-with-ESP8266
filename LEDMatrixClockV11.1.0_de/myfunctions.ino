...
sendCmdAll(CMD_SHUTDOWN,1); Ghost(); // Matrix einschalten und Ghost anzeigen
...
if (SerialMonitor) {DEBUG_PRINTLN(F("Lade Einstellungen der Uhr ..."));}
  ...
  if (NTPSN != "") {ntpServerName = NTPSN.c_str();} else {ntpServerName = "de.pool.ntp.org";} // Default
...
// Bundesland im Speicher löschen (z.Bsp. bei Umzug nach Österreich)
  ...
  LoadClockStatus(); // letzten Status der Uhr laden
    ...
    ntpServerName = "de.pool.ntp.org";  // Finde lokale Server unter http://www.pool.ntp.org/zone
    }
...
void SaveMatrixAdjustments(){
  ...
  // Wird nicht mehr gebraucht und wird gelöscht wenn vorhanden
...
long WiFiRSSI() {
const int8_t N = 20; // Anzahl Messungen für den Mittelwert
...
// 1. Messung
long rssi = WiFi.RSSI();
// 2. In Ringpuffer schreiben
rssiBuffer[rssiIndex] = rssi;
rssiIndex = (rssiIndex + 1) % N;  // Ringpuffer-Logik
if (!bufferfull) bufferfull = (rssiIndex == 0);
// 3. Mittelwert berechnen
count = bufferfull ? N : rssiIndex;  // wie viele Werte sind schon gültig
for (byte i = 0; i < count; i++) {
sum += rssiBuffer[i];} MyWaitLoop(5);} // kurz warten
...
long WifiRSSI = WiFiRSSI(); // WiFi Signalstärke in dBm
...
if (SHT == "sehr gut" || SHT == "gut") {return true;} else {return false;}}

// *************************************************************************************************

void LoadClockStatus() {   
if (LittleFS.exists("/clockstatus.txt")){
ClockStatus = SetupLoad("clockstatus.txt");
...
bool ByteRAM = false;
if (Modul == "GMC" || Modul == "Gmc" || Modul == "gmc") {MinRAM = 16312;} // GMC braucht mindestens 16312 Byte RAM
if (SerialMonitor) {
DEBUG_PRINTLN(F(""));
