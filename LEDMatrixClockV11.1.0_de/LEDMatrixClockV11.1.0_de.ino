 ...
 ********************************************************************************************
*/
const char* LMCVersion = "11.1.0 de";
...
// ##########################################################################################################################################################
byte ScrollClockTime  = 80; // Scrollverzögerung bei Animation der Uhrzeit    80 = Default
// ##########################################################################################################################################################

// #################### Temperatur- und Luftfeuchtesensor (DHT 11 oder DHT 22) ##############################################################################
String DHTSensor = "NoDHT"; // DHT11 / DHT22 / NoDHT    Sensor    (Ausgabe der Temperatur in Celsius) - Ab Version 10.2.9 im Captiv Portal änderbar
// ##########################################################################################################################################################

// ########################## Mitternacht einen Geist anzeigen ###################### kann ab Version 10.1.0 im Captive Portal geändert werden ##############
String MidnightGhost = "JA"; // JA = Geist anzeigen / NEIN = Geist nicht anzeigen
// ##########################################################################################################################################################
...
// ##########################################################################################################################################################
int16_t MinRAMMem   = 15872; // Minimum RAM in Byte  Default = 15872 
// ##########################################################################################################################################################

// ########################## GMCMapMidnightAccessDelay >> Default = 30. Minute | Minimum = 1 | Maximum = 59 ################################################
...
// ##########################################################################################################################################################
String NTPSN = "de.pool.ntp.org";
const char* ntpServerName = "de.pool.ntp.org";  // Finde lokale Server unter http://www.pool.ntp.org/zone
// ##########################################################################################################################################################
...
// ##########################################################################################################################################################
byte MaxWiFiConnectLoop = 20;  // Maximale Anzahl Loops bei Verbindung mit dem WLAN  -  Minimum = 15  Maximum = 254
// ##########################################################################################################################################################
...
String StartDate; // Startdatum der Uhr 
...
CheckGlobalVariableRange(); // Check Variablen
...
// #####################################################################################
  ...
  StartDate = FormatedDateTimeDE(true); // Startdatum / Uhrzeit
...
printStringWithShift("True", ScrollTextTime);} else {printStringWithShift("False", ScrollTextTime);}*/
...
if (SHT == "schlecht") { 
SHT = cleanText("       +++     Achtung: Die WiFi Signalstärke ist schlecht !    ---    Die Uhr wird ohne weitere Online Funktionen gestartet...       +++                    ");
...
if (GMCMapMidnightAccessDelay(GMCMMAD)) {  
if (IsEnoughRAM(MinRAMMem,"GMC") && IsWiFiSignalOK()) {  // nur wenn genügend RAM zur Verfügung steht und WiFiSignal OK 
...
await = false;} // Start der Uhr - Ende
  ...
  if (IsEnoughRAM(MinRAMMem,"Wetter") && IsWiFiSignalOK()) {getWeatherData();}} // Neuen Sonnenauf- und Sonnenuntergang ermitteln
  PDP = false; // Doppelpunkt permanet wieder aus wenn Blinken Modus
  if (Display) {getTimeLocal(); ClockScrollIn();} 
  await = false;}}
  ...
  getGMCMapData(); // aktuelle Radioaktivitätswerte holen
  printStringWithShift("             ", ScrollTextTime); // GMCMap aus dem Display laufen lassen
  GMCMapMessage(5,2);}}} // GMCMap Daten Auswerten und Anzeigen oder RAM defragmentieren
  ClockScrollIn(); // Uhrzeit in Display hineinscrollen
  ...
  if (MatrixAction) { // 10 Minuten vor dem automatischen Abschalten der Matrix keine Aktionen mehr ! 
  ...
  getGMCMapData(); // aktuelle Radioaktivitätswerte holen
  printStringWithShift("             ", ScrollTextTime); // GMCMap aus dem Display laufen lassen
  GMCMapMessage(10,2);}}} // GMCMap Daten Auswerten und Anzeigen oder RAM defragmentieren
  ...
  if  (String(m)+":"+String(s) == OnlyTempViewTime[i]+":11") {    
    ...
    // *** Außentemperatur ***
    if (WeatherFunctions && AccessOpenWeatherMap){
    if (IsEnoughRAM(MinRAMMem,"Wetter") && IsWiFiSignalOK()) {  // nur wenn genügend RAM zur Verfügung steht und WiFiSignal OK   
...
// ----------- nur Innen- und Außenluftfeuchte  ---------------------- 
  ...
  if  (String(m)+":"+String(s) == OnlyHumViewTime[i]+":11") { 
    ...
    if (StatusOpenWeatherMap == "OK") { MyWaitLoop(10000);} // kurz warten (10 sek. - default) oder RAM defragmentieren
  ...
  if (IsEnoughRAM(MinRAMMem,"News") && IsWiFiSignalOK()) {getNews();} else {APINews.clear();}} // aktuelle News
    // ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    SIC = true; // ScrollIn Clock  
    printStringWithShift("          ", ScrollTextTime);
    printStringWithShift(APINews.c_str(), ScrollTextTime);}} // Anzeige News oder RAM defragmentieren
    // ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    if (SIC) {ClockScrollIn(); SIC = false;} // Uhrzeit in Display hineinscrollen 
  ...
  getWeatherData();  // aktuelle Wetterdaten holen
  printStringWithShift("          ", ScrollTextTime);
  printStringWithShift(weatherString.c_str(), ScrollTextTime);} // Uhrzeit in Display hineinscrollen oder RAM defragmentieren
  if (SIC) {ClockScrollIn(); SIC = false;} // Uhrzeit in Display hineinscrollen 
  PDP = false; // Doppelpunkt permanet wieder aus wenn Blinken Modus
  await = false;}}}
