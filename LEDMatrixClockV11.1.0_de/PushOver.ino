...
DEBUG_PRINTLN(F("Verbindung zum  PushOver.net - Server herstellen..."));}
...
if (SerialMonitor) {
DEBUG_PRINTLN(F(""));
DEBUG_PRINTLN("[Pushover] - Sende Nachricht: "+POMessage); DEBUG_PRINTLN(F(""));}
int16_t httpCode = http.POST(postmessage);
if (SerialMonitor) {DEBUG_PRINTLN("[HTTP] POST... code: " + HTTPCodeText(httpCode));}
if(httpCode == HTTP_CODE_OK) {
if (SerialMonitor) {DEBUG_PRINTLN(F("[Pushover] - Die Nachricht wurde erfolgreich gesendet"));}} else {  
if (SerialMonitor) {DEBUG_PRINTLN(F("[Pushover] - Die Nachricht konnte nicht gesendet werden !"));
DEBUG_PRINTLN(F(""));} FailPushOver = true;}
http.end();
client.stop();
} else {
client.stop();
if (SerialMonitor) {DEBUG_PRINTLN(F("[Pushover] - Verbindung zum Dienst fehlgeschlagen !"));
DEBUG_PRINTLN(F("")); FailPushOver = true;}}
}
...
if (IsEnoughRAM(MinRAMMem,"PushOver") && IsWiFiSignalOK()) {  // nur wenn genügend RAM zur Verfügung steht und WiFiSignal OK      
String MPushOverMessage; int16_t NIHS;
MPushOverMessage.clear();
if (IsNumeric(IHS)) {NIHS = (int16_t)strtol(IHS.c_str(), NULL, 10);} else {NIHS = -1;}
...
if (SerialMonitor) {DEBUG_PRINTLN("Pushover Nachricht an "+PushoverDevice1+" senden");}
if (NIHS >= 90 ) { 
...
DEBUG_PRINTLN("Die Pushover Nachricht an "+PushoverDevice1+" konnte nicht gesendet werden !");} 
...
if (SerialMonitor) {DEBUG_PRINTLN("Pushover Nachricht an "+PushoverDevice2+" senden");}
if (NIHS >= 90 ) {  
printStringWithShift("             P-Over", ScrollTextTime);   
SendPushoverMessage(MPushOverMessage,PushoverDevice2,"siren"); // Nachricht an Gerät 2 senden
if (SerialMonitor && FailPushOver) {
DEBUG_PRINTLN("Die Pushover Nachricht an "+PushoverDevice2+" konnte nicht gesendet werden !");}
printStringWithShift("             ", ScrollTextTime);} else {
printStringWithShift("             P-Over", ScrollTextTime);      
...
if (SerialMonitor) {DEBUG_PRINTLN("Pushover Nachricht an "+PushoverDevice3+" senden");}
if (NIHS >= 90 ) {  
printStringWithShift("             P-Over", ScrollTextTime);   
SendPushoverMessage(MPushOverMessage,PushoverDevice3,"siren"); // Nachricht an Gerät 3 senden
if (SerialMonitor && FailPushOver) {
DEBUG_PRINTLN("Die Pushover Nachricht an "+PushoverDevice3+" konnte nicht gesendet werden !");}
printStringWithShift("             ", ScrollTextTime);} else {
printStringWithShift("             P-Over", ScrollTextTime); 
...
if (SerialMonitor) {DEBUG_PRINTLN("Die WiFi Signalstärke ist zu niedrig !");}} else {    
if (!await){ 
await = true; 
PushOverTest = true;   
if (SerialMonitor) {DEBUG_PRINTLN("Pushover Testnachricht an "+PushoverDevice1+" senden ...");} 
printStringWithShift("             ", ScrollTextTime);
printStringWithShift("P-Over", ScrollTextTime); 
...
PushOverTest = false;
await = false;} else {HandleOTAandWebServer();}}}
...
await = true;
PushOverTest = true;  
if (SerialMonitor) {DEBUG_PRINTLN("Pushover Testnachricht an "+PushoverDevice2+" senden ...");} 
printStringWithShift("             ", ScrollTextTime);
printStringWithShift("P-Over", ScrollTextTime); 
...
await = true;
PushOverTest = true;  
if (SerialMonitor) {DEBUG_PRINTLN("Pushover Testnachricht an "+PushoverDevice3+" senden ...");} 
printStringWithShift("             ", ScrollTextTime);
printStringWithShift("P-Over", ScrollTextTime); 
