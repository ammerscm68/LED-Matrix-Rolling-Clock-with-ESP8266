...
OnlyReboot = true; ResetClockSettings(); OnlyReboot = false;}} // Nur neu starten statt Reset
...
if (littleFSInit && ResetNow) {ResetClock();}} else { // Einstellungen auf Standard - WLAN Zugangsdaten werden gelöscht
HTMLAnswer("PW");}} 
...
HTMLString += F("<a><br /></a>");
HTMLString += "<h7>Startdatum: &nbsp;&nbsp;"+StartDate+"</h7>";
HTMLString += F("<a><br /></a>");
if (ClockStatus == "OK") {
...
HTMLString += F("<h4>Bitte warten ...</h4>");
if (Display) {HTMLString += F("<h3>(Bis die Uhrzeit wieder im Matrix-Display angezeigt wird)</h3>");}
...
HTMLString += F("<a><br /></a>");
if (OnlyReboot) {
HTMLString += F("<h3>Zu wenig Arbeitsspeicher !</h3>");
HTMLString += F("<h2>(Ein Neustart der Uhr behebt das Problem)</h2>");
HTMLString += F("<a><br /></a>");
HTMLString += F("<h2>Jetzt LED-Matrix-Uhr neu starten ?</h2>");
HTMLString += F("<a><br /></a>");
HTMLString += F("<a href =\"/reboot\"><button class=\"button\" style=\"height: 48px; width: 110px; font-size: 22px; color: red\">JA</button></a>");  
} else {
HTMLString += F("<h3>LED-Matrix-Uhr zurücksetzen !</h3>");
HTMLString += F("<h2>(Die WLAN-Zugangsdaten werden gelöscht)</h2>");
HTMLString += F("<a><br /></a>");
HTMLString += F("<h2>Jetzt LED-Matrix-Uhr zurücksetzen ?</h2>");
HTMLString += F("<a><br /></a>");
HTMLString += F("<a href =\"/goreset\"><button class=\"button\" style=\"height: 48px; width: 110px; font-size: 22px; color: red\">JA</button></a>");}
HTMLString += F("<a href =\"/\"><button class=\"button\" style=\"margin-left: 80px;height: 48px; width: 110px; font-size: 22px;\">NEIN</button></a>");
HTMLString += F("</body>");
HTMLString += F("</html>");  
OneTimeCall = true; 
SendHTMLString(HTMLString); // Send HTMLString
...
HTMLString = F("<!DOCTYPE html> <html lang='de'>\
  ...
  <style>\
   ...
   h3 {font-size: 14px; margin-top: 0px; margin-bottom: 50px;}\
    ...
    <a><br /></a>\
    <h2>Diese Seite wurde nicht gefunden !</h3>\
...
if (server.method() == HTTP_POST) {
    ...
    if (MidnightGhost == "JA" && DisplayOffTime == "00:00") {
    ...
    MyWaitLoop(100);}} else {HTMLAnswer("PW");}}  // Send HTMLString oder Warten
...
await = false;
if (SerialMonitor) {DEBUG_PRINTLN(F("Summer (Buzzer) Stundensound eingeschaltet"));}} else {HTMLAnswer("PW");}
