...
bool DHT11Sensor = true; // true = DHT11  /  false = DHT22   Sensor
// ####################################################################################################
...
// #####################################################################################################
const int PushButtonPin = D3; // PushButton-Pin (D3 = GPIO 0)
const int buzzerPin = D2; // passiv Buzzer Init || for ESP8266 Microcontroller (D4 dont work with ESP8266 Microcontroller)
// #####################################################################################################

// ###### Anzeigezeiten für "OpenWeatherMAP" und "NewsAPI" sowie Innentemperatur und Luftfeuchte (DHT-Sensor) ###
...
// Vor Aktion Doppelpunkt Permanent anzeigen
int DoublePointPermanantTimeCount = 11;
String DoublePointPermanantTime[] = {"5", "10", "15", "20", "25", "30", "35", "40", "45", "50", "55"}; 
//###############################################################################################################
...
// #####################################################################################
  ...
  if (InTempValue != "-") {
  Serial.println("");  
  Serial.println("DHT-Sensor ist angeschlossen. ("+InTempValue+"°C)");
  Serial.println("");} else {
  Serial.println("");  
  Serial.println("kein DHT-Sensor angeschlossen oder Sensor defekt !");
  Serial.println("");}}  
  ...
  //saveing millis after setup for Runtime Modul - WLAN Verbindung OK
  ...
  else { // keine gespeicherten Daten - Access Point (Captive Portal) starten
  if (SerialMonitor) {  
...
await = true; 
// **** Ein Hallo beim Start an den/die Besitzer ***
if (WatchOwner1 != "") {
String SHT= "               Hallo  "+WatchOwner1+"                 "; 
printStringWithShift(SHT.c_str(), ScrollTextTime);}
if (WatchOwner2 != "") {
String SHT= "               Hallo  "+WatchOwner2+"                 "; 
printStringWithShift(SHT.c_str(), ScrollTextTime);}
if (WatchOwner3 != "") {
String SHT= "               Hallo  "+WatchOwner3+"                 "; 
printStringWithShift(SHT.c_str(), ScrollTextTime);}
// *************************************************
sendCmdAll(CMD_SHUTDOWN,0); // Display aus
...
await = false;
  ...
  printStringWithShift("        +++       Die Luftfeuchtigkeit am Standort der Uhr ist zu niedrig ... !      +++           ",ScrollTextTime);
  ClockScrollIn();}
  if (IntInHumValue >= 68) {
  dots = 1; // Doppelpunkt an
  getAnimClock(true); // Uhrzeit anzeigen / berechnen 
  MyWaitLoop(1500);  
  // Luftfeuchte zu hoch
  String HumMessageString = "        +++       Die Luftfeuchtigkeit am Standort der Uhr ist zu hoch ... !      +++           ";
