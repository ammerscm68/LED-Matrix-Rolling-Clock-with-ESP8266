...
DEBUG_PRINTLN(F("Connect to the PushOver.net Server..."));}
...
if (SerialMonitor) {
DEBUG_PRINTLN(F(""));
DEBUG_PRINTLN("[Pushover] - Send Message: "+POMessage); DEBUG_PRINTLN(F(""));}
int16_t httpCode = http.POST(postmessage);
if (SerialMonitor) {DEBUG_PRINTLN("[HTTP] POST... code: " + HTTPCodeText(httpCode));}
if(httpCode == HTTP_CODE_OK) {
if (SerialMonitor) {DEBUG_PRINTLN(F("[Pushover] - The Message was sent successfully"));}} else {  
if (SerialMonitor) {DEBUG_PRINTLN(F("[Pushover] - The Message could not be sent !"));
DEBUG_PRINTLN(F(""));} FailPushOver = true;}
http.end();
client.stop();
} else {
client.stop();
if (SerialMonitor) {DEBUG_PRINTLN(F("[Pushover] - Connection to Service failed !"));
DEBUG_PRINTLN(F("")); FailPushOver = true;}}
}
...
if (IsEnoughRAM(MinRAMMem,"PushOver") && IsWiFiSignalOK()) {  // Only if enough RAM is available     
...
if (SerialMonitor) {DEBUG_PRINTLN("Send Pushover Message to "+PushoverDevice1);}
if (NIHS >= 90 ) {
...
DEBUG_PRINTLN("The Pushover Message to "+PushoverDevice1+" could not be sent!");} 
...
if (SerialMonitor) {DEBUG_PRINTLN("Send Pushover Message to "+PushoverDevice2);}
if (NIHS >= 90 ) {
printStringWithShift("             P-Over", ScrollTextTime);   
SendPushoverMessage(MPushOverMessage,PushoverDevice2,"siren"); // Send Message to Device 2
if (SerialMonitor && FailPushOver) {
DEBUG_PRINTLN("The Pushover Message to "+PushoverDevice2+" could not be sent!");}
printStringWithShift("             ", ScrollTextTime);} else {
printStringWithShift("             P-Over", ScrollTextTime);      
...
if (SerialMonitor) {DEBUG_PRINTLN("Send Pushover Message to "+PushoverDevice3);}
if (NIHS >= 90 ) {
printStringWithShift("             P-Over", ScrollTextTime);   
SendPushoverMessage(MPushOverMessage,PushoverDevice3,"siren"); // Send Message to Device 3
if (SerialMonitor && FailPushOver) {
DEBUG_PRINTLN("The Pushover Message to "+PushoverDevice3+" could not be sent!");}
printStringWithShift("             ", ScrollTextTime);} else {
printStringWithShift("             P-Over", ScrollTextTime); 
...
if (SerialMonitor) {DEBUG_PRINTLN("The WiFi Signal Strength is too low !");}} else {  
if (!await){ 
await = true;
PushOverTest = true;
if (SerialMonitor) {DEBUG_PRINTLN("Send Pushover Test Message to: "+PushoverDevice1);}
printStringWithShift("             ", ScrollTextTime); 
printStringWithShift("P-Over", ScrollTextTime); 
...
await = true;
PushOverTest = true;
if (SerialMonitor) {DEBUG_PRINTLN("Send Pushover Test Message to: "+PushoverDevice2);} 
printStringWithShift("             ", ScrollTextTime);
printStringWithShift("P-Over", ScrollTextTime); 
...
await = true;
PushOverTest = true;
if (SerialMonitor) {DEBUG_PRINTLN("Send Pushover Test Message to: "+PushoverDevice3);} 
printStringWithShift("             ", ScrollTextTime);
printStringWithShift("P-Over", ScrollTextTime); 
