...
bool MDTime,FDisplayOnTime,FDisplayOffTime; // Hilfsvariablen
  ...
  InTempOffset = SetupLoad("dht-intempoffset.txt");} else {InTempOffset = "-1";}  // Temperature
  InTempOffset.trim(); // Leerzeichen vorne und Hinten entfernen
  if (LittleFS.exists("/dht-inhumoffset.txt")){
  InHumOffset = SetupLoad("dht-inhumoffset.txt");} else {InHumOffset = "-1";}  // Humidity
...
// Check DHT Offset
if (!IsNumeric(InTempOffset)) {InTempOffsetInt = -1;} else {InTempOffsetInt = InTempOffset.toInt();} // Temperature
if (!IsNumeric(InHumOffset)) {InHumOffsetInt = -1;} else {InHumOffsetInt = InHumOffset.toInt();} // Humidity
InTempOffset = (String)InTempOffsetInt; 
InHumOffset = (String)InHumOffsetInt; 
InTempOffset.toCharArray(WMInTempOffset, 4);
InHumOffset.toCharArray(WMInHumOffset, 4);
...
Serial.println("[Save-DHT] InTempOffset: " + (String)WMInTempOffset);  // Only for Debuging
Serial.println("[Save-DHT] InHumOffset: " + (String)WMInHumOffset);  // Only for Debuging*/
  ...
  // Check DHT Offset
  if (!IsNumeric((String)WMInTempOffset) || (String)WMInTempOffset == "") {InTempOffset = "-1";} // Temperature
  if (!IsNumeric((String)WMInTempOffset) || (String)WMInTempOffset == "") {InHumOffset = "-1";} // Humidity
  
  // DHT-Sensor Offset
  if ((String)WMInTempOffset != ""){
  SetupSave("dht-intempoffset.txt", (String)WMInTempOffset);} else {
  if (LittleFS.exists("/dht-intempoffset.txt")){LittleFS.remove("/dht-intempoffset.txt");}}
  if ((String)WMInHumOffset != ""){
  SetupSave("dht-inhumoffset.txt", (String)WMInHumOffset);} else {
...
/*if  ((String(d) + "." + String(mo)) == "17.3") // Geburtstagsgruß
{
GreetingsToday = true;
// Serial.println(String(h)+":"+String(m)+":" + String(s)+" --- Alles gute zum Geburtstag liebe Rosi");
printStringWithShift("       +++     Alles gute zum Geburtstag      +++                ", ScrollTextTime);
}
