...
HTMLString += F("<a><br /></a>");
if (!MEP && !await) {
HTMLString += "<h7>Durée d'exécution de l'Horloge: &nbsp;&nbsp;"+rtime+"</h7>";} else {
HTMLString += F("<h7>Durée d'exécution de l'Horloge: &nbsp;&nbsp; S'il vous plaît, attendez ...</h7>");}
...
if (WebFunction == "NewsDataFunctionON1")  {HTMLAnswerString = F("NewsData.io - Actualités actuelles a été Commuté ALLUMÉ<br />Lorsque vous Revenez à la Page d'accueil<br />l'Horloge Matricielle LED Redémarrera");}} else {
...
if (AccessGMCMap && AccessNewsData) {
if (WebFunction == "GMCMapFunctionON1")  {HTMLAnswerString = F("GMCMap.com - Les Niveaux de Radioactivité actuels ont été activés<br />Lorsque vous Revenez à la Page d'accueil<br />l'Horloge Matricielle LED Redémarrera");}} else {
...
// ++++++++++++++++++++++++++++++++++++++++++++++ 
if (AccessGMCMap && AccessNewsData) {  
GMCMapFunction = false; // GMC désactivé en raison d'un manque de mémoire
...
if (SerialMonitor) {
Serial.println(F("NewsData - Les actualités actuelles ont été activées"));
if (AccessGMCMap && AccessNewsData) {Serial.println(F("+++   L'Horloge redémarrera   +++"));}}
...
if (SerialMonitor) {Serial.println(F("NewsData - Les actualités actuelles sont désactivées"));}
} else {PleaseWait();}}
...
// +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ 
if (AccessGMCMap && AccessNewsData) {
NewsDataFunction = false;  // Désactiver les actualités en raison d'un manque de mémoire
StatusNewsData = F("disabled");
...
if (SerialMonitor) {
Serial.println(F("GMCMap - Les Niveaux de Radioactivité actuels ont été activés"));
if (AccessGMCMap && AccessNewsData) {Serial.println(F("+++   L'Horloge redémarrera   +++"));}}
HTMLAnswer("GMCMapFunctionON1");} else {
HTMLAnswer("GMCMapFunctionON2");
if (SerialMonitor) {Serial.println(F("GMCMap - Les Niveaux de Radioactivité actuels sont déjà activés"));}} 
} else {PleaseWait();}}
...
if (SerialMonitor) {Serial.println(F("GMCMap - Affichage actuel de la Radioactivité éteint"));}
} else {PleaseWait();}}
...
if (SerialMonitor) {Serial.println(F("Fonctions supplémentaires activées - (Date, Anniversaires, Jours Fériés)"));} 
} else {PleaseWait();}}
...
if (SerialMonitor) {Serial.println(F("Fonctions Supplémentaires désactivées - (Date, Anniversaires, Jours Fériés)"));}
} else {PleaseWait();}}
...
HTMLString += F("<a><br /></a>");}
if (MidnightGhost == "YES") {HTMLString += F("<h2>Afficher les Fantômes à Minuit ?:  &nbspYES</h2>");} else {HTMLString += F("<h2>Afficher les Fantômes à Minuit ?:  &nbspNO</h2>");}
HTMLString += F("<a><br /></a>");
