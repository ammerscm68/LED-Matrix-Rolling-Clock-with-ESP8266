 ...
 *******************************************************************************************
*/
const char* LMCVersion = "11.0.5 uk";
...
// #############################################################################################################################################
String NTPSN = "uk.pool.ntp.org";
const char* ntpServerName = "uk.pool.ntp.org";  // Finde lokale Server unter http://www.pool.ntp.org/zone
// #############################################################################################################################################
...
byte GNCounter = 1; // Counter for Greetings and News
...
bool PDP = false;
bool EnhancedFunctions = true; // Additional Functions ON / OFF - Standard = ON (Date, Birthdays, etc.)
...
// *************************************************************************************************************************************************************
MEP=true; Greetings(true); MEP=false; // First, greetings if available 
...
await = false;} // Start of the clock - End
  ...
  getTimeLocal(); 
  if (EnhancedFunctions) {MEP=true; Greetings(false); MEP=false; // Determine Greetings for the new Day  
  ...
  // Additional Functions (Date, Birthdays) and/or News Displays --- every 10 Minutes
  ...
  if  (String(m)+":"+String(s) == DateNewsViewTime[i]+":11") { 
    ...
    if (GNCounter == 0) {MEP=true; Greetings(false); MEP=false;} else {GreetingsToday = false;}
    GNCounter++;
    if (GNCounter > 1) {GNCounter = 0;}} // Alternately display Greetings as well as News
    // ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
