...
String rtime = ""; // Temps de fonctionnement de l'Horloge
...
static const char ntpServerName[] = "pool.ntp.org";    // Recherchez le serveur local sous http://www.pool.ntp.org/zone
...
void loop() {  
 ...
 if (!await) {MatrixButtonOnOff();} 
  ...
  printStringWithShift(ct.c_str(),ScrollTextTime);}
  MyWaitLoop(1000);}} else { // 1 sek. attendez
...
await = false;} // Début de l'Horloge - Fin
  ...
  printStringWithShift("        +++       Attention : L'Humidité à l'emplacement de la Montre est trop faible... !      +++           ",ScrollTextTime);
  MyWaitLoop(1500);}}  
  ...
  for (int i=0; i <= 1; i++){ // Répétez le Message deux fois
  printStringWithShift("        +++       Attention : L'Humidité à l'emplacement de la Montre est trop élevée... !      +++           ",ScrollTextTime);
  MyWaitLoop(1500);}}
  if (InHumidityStatus != "MIN" && InHumidityStatus != "MAX") {StatusWarningLED("LOW");} // LED éteinte
