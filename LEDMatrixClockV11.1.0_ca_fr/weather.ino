...
DEBUG_PRINTLN(F("Téléchargement des données Météorologiques - Veuillez Patienter ..."));}
   ...
   while (!http.connected()) {ConnectCount += 1;MyWaitLoop(100);if (ConnectCount > 45) {break;}} // attends brièvement
    ...
    SaveClockStatus("Erreur de filtrage JSON météo:");
    http.end(); client.stop(); filter.clear(); ESP.restart();} else { 

    // DynamicJsonDocument doc(256) ;  // Document Json
    ...
    SaveClockStatus("Erreur de document JSON météo: ( "+String(error.c_str())+" )");
    http.end(); // Quitter le client HTTP
    client.stop(); doc.clear(); filter.clear(); ESP.restart(); // Fermer le client Wi-Fi
    } else { 

    doc.shrinkToFit();
    // doc.garbageCollect(); // N'est plus disponible à partir d'ArduinoJSON 7.
    http.end(); // // Quitter le client HTTP
    client.stop(); // Fermer le client Wi-Fi

    // Attribuer des données météorologiques
    ...
    WPweatherString = "+++ Les données Météorologiques Actuelles pour " + weatherLocation + ":    ---    "; // Nom de la ville
    ...
    DEBUG_PRINTLN("Les données Météorologiques actuelles pour " + weatherLocation+":");
