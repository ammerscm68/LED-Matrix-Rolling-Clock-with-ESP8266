...
MyWaitLoop(500); //wait     
sendCmdAll(CMD_SHUTDOWN,0); // Display OFF
if (SerialMonitor) {Serial.println("Matrix switched off via Button at "+ FormatedDateTimeUS(false));}} else { 
APStart += 1; // inc   
...
if (!Start) { 
    ...
    // ShortBuzzerBeep();
    if (SerialMonitor) {Serial.println("Matrix switched on via Button at "+ FormatedDateTimeUS(false));}}
    APStart += 1; // inc 
...
if (SerialMonitor) {Serial.println("UDP Server started - "+FormatedDateTimeUS(false));}}

// *************************************************************************************************

void WiFiUDPStop() {
Udp.stop();
if (SerialMonitor) {Serial.println("UDP Server stopped - "+FormatedDateTimeUS(false));}}

// *************************************************************************************************

void WebServerStart() {
server.begin();
if (SerialMonitor) {Serial.println("Web Server started - "+FormatedDateTimeUS(false));}}

// *************************************************************************************************

void WebServerStop() {
server.stop();
if (SerialMonitor) {Serial.println("Web Server stopped - "+FormatedDateTimeUS(false));}}
