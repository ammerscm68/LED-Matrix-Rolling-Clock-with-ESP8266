...
// Humidité intérieure – Message si elle est trop faible ou trop élevée
String RLF = F("");
if (InHumValue != "!") {
if (InTempOffset == "999") {InHumValue = "0";} else {InHumValue = InHumMessure();} // Récupérer la valeur de l'humidité de l'air
MyWaitLoop(100); // courte attente
  ...
  InHumMessage = F("Attention : Humidité de l'Air trop faible !");}
  if (IntInHumValue >= MaxComfortHum) {
  RLF = F("MAX");  
  // Humidité de l'air trop élevée     
  if (Display) {digitalWrite(HWLEDPin, HIGH);} else {digitalWrite(HWLEDPin, LOW);}  // LED switch On or Off 
  if (WeatherFunctions and Display){
  if (humidity >= MaxComfortHum || humidity == -1) {InHumMessage = F("Attention : Humidité de l'Air trop élevée !");} else 
  {InHumMessage = F("Attention : Humidité de l'Air trop élevée ! - Merci d'aérer");}
  } else {InHumMessage = F("Attention : Humidité de l'Air trop élevée !");}}
  if (IntInHumValue >= MinComfortHum && IntInHumValue <= MaxComfortHum) {
  RLF = F("NORMAL");  
  digitalWrite(HWLEDPin, LOW); // LED éteinte
  InHumMessage = F("");}
  } else {RLF = F("NORMAL"); InHumMessage = F("Capteur DHT désactivé !"); digitalWrite(HWLEDPin, LOW);} // LED éteinte
  } else {
  digitalWrite(HWLEDPin, LOW); // LED éteinte
  InHumValue = F("!");
  RLF = F("FAIL");
  InHumMessage = F("");}} else {digitalWrite(HWLEDPin, LOW); RLF = F("FAIL");} // La LED s'éteint et échoue
  return RLF;  
}
