...
if (server.method() == HTTP_POST) {
    ...
    if (NTPSN != "") {ntpServerName = NTPSN.c_str();} else {ntpServerName = "us.pool.ntp.org";}
...
int n = WiFi.scanNetworks(false, true); //WiFi.scanNetworks(async, show_hidden)
    ...
    HTMLString += F("<br/>"); 
    HTMLString += F("<br/>");
    HTMLString += "<div class='form-floating'><label>NTP-Serverpool (Example: us.pool.ntp.org)</label><input type='text' class='form-control' name='NTPS' id='NTPS' value='"+NTPSN+"'></div>";
    HTMLString += F("<br/>"); 
    // *********************************** Select Time Zone **************************************************************************************************************
