void DHTHTML() { 
  ...
  if (IntInTempValue >= 10 && IntInTempValue <= 28) {TempValueColor = "<tr><td style=\"height: 10px;\">Current Indoor Temperature:&nbsp&nbsp</td><td id='Gvalue'><span>"+InTempValue+"</span>°C</td></tr>";} else {  
...
if (InTempValue != "!") {  
  ...
  if (SerialMonitor) {Serial.println("Indoor Temperature: "+ITempValue+" °C");} // only Debugmodus
...
MyWaitLoop(100); // short wait
  ...
  if (Display) {StatusWarningLED("HIGH");} else {StatusWarningLED("LOW");}  // LED switch On or Off
  ...
  InHumMessage = F("");}
  } else {RLF = F("NORMAL"); InHumMessage = F("DHT-Sensor disabled !"); StatusWarningLED("LOW");} // LED switch off
  } else {
  StatusWarningLED("LOW"); // LED switch off
  InHumValue = F("!");
  RLF = F("FAIL");
  InHumMessage = F("");}} else {StatusWarningLED("LOW"); RLF = F("FAIL");} // LED switch off and Fail
  return RLF;  
}
...
if (server.method() == HTTP_POST) {
    ...
    if (SerialMonitor) {Serial.println(F("DHT Sensor offset Data saved... "));}
    // Website anzeigen  
    HTMLString = F("<!doctype html><html lang='en-UK'>");
    HTMLString += F("<head><meta charset='utf-8'>");
    HTMLString += F("<meta name='viewport' content='width=device-width, initial-scale=1'>");
