...
// contrast(); //AutoBrightness
 ...
 if (!await) {MatrixButtonOnOff();} 
  ...
  if (SerialMonitor) {Serial.println("Please wait ... "+FormatedDateTimeUS(false));}
...
if (SHT == "bad") { 
SHT = cleanText("       +++     Attention: The WiFi Signal Strength is bad !    ---    The Clock will still Start...       +++                    ");}   
if (SHT == "very bad") {  
...
await = false;} // Start of the clock - End
  ...
  if (IsPushover) {PushOverAlert(InHumidityStatus);}  // Send Message via Pushover 
  ...
  if  (String(m)+":"+String(s) == OnlyTempViewTime[i]+":11") {    
    ...
    StatusWarningLED("LOW"); // LED turn off
    InTempValue = "-";}} else {StatusWarningLED("LOW");} // LED turn off
