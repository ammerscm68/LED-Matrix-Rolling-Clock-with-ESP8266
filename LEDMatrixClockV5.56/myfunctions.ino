...
bool MDTime; // Hilfsvariable
  ...
  if (LittleFS.exists("/newsapi-apikey.txt")){
   newsKey = SetupLoad("newsapi-apikey.txt");} else {newsKey = "";} 
  ...
  InTempOffset = SetupLoad("dht-intempoffset.txt");} else {InTempOffset = "0";}  // Temperature
  InTempOffset.trim(); // Leerzeichen vorne und Hinten entfernen
  if (LittleFS.exists("/dht-inhumoffset.txt")){
  InHumOffset = SetupLoad("dht-inhumoffset.txt");} else {InHumOffset = "0";}  // Humidity
...
// Serial.println("*** Display On Time *** "+DisplayOnTime);
// Serial.println("*** Display Off Time *** "+DisplayOffTime);
