...
time_t getNtpTime() {
  ...
  if (SerialMonitor) {Serial.println("NTP-Time - No NTP response - "+FormatedDateTimeUS(false));}
  // return 0; // returns 0 if the time cannot be determined.
  printStringWithShift("        +++        The NTP Server is not responding !      +++           ",ScrollTextTime);
  // Restart Clock
  if (SerialMonitor) {Serial.println("The Clock will Restart - "+FormatedDateTimeUS(false));}
  printStringWithShift("        +++       The Clock will Restart      +++           ",ScrollTextTime);
  ESP.restart();
  return false;}
