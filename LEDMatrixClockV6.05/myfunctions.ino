...
if (littleFSInit == true) {
  ...
  // Display Ein- und/oder Ausschaltzeit wurde geändert ?
  if (LittleFS.exists("/display-change-time.txt")){
  DisplayTimeChange = SetupLoad("display-change-time.txt");
  } else {DisplayTimeChange = "False";} 
...
if (!MDTime) {DisplayOffTime = "---";}}}}}  
  ...
  // Serial.println("*** Display On Time ***: "+DisplayOnTime+" Uhr");
  // Serial.println("*** Display Off Time ***: "+DisplayOffTime+" Uhr");
