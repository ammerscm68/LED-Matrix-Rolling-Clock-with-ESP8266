...
if (server.method() == HTTP_POST) {
    ...
    DisplayOnTime = ""; // Funktion erstmal deaktiviert
    DisplayOffTime = ""; // Funktion erstmal deaktiviert
    ...
    if (SerialMonitor) {Serial.println("Uhr wird neu gestartet");} 
...
int n = WiFi.scanNetworks(false, false); //WiFi.scanNetworks(async, show_hidden)
    ...
    if (InTempValue != "-") {
    HTMLString += "<div class='form-floating'><br/><label>DHT-Sensor Temperatur Offset (Beispiel: -1 or +1)</label><input type='text' maxlength='3' class='form-control' name='DHTTo' id='DHTTo' value='"+InTempOffset+"'></div>";
    HTMLString += "<div class='form-floating'><br/><label>DHT-Sensor Luftfeuchte Offset (Beispiel: -1 or +1)</label><input type='text' maxlength='3' class='form-control' name='DHTHo' id='DHTHo' value='"+InHumOffset+"'></div>";}
    if (IsBuzzer) {
