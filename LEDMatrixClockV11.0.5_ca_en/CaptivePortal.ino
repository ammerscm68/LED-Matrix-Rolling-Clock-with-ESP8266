...
if (server.method() == HTTP_POST) {
    ...
    DHTSensor = server.arg("DHT"); // DHT Sensor Selection [DHT11 or DHT22]
    MCTZ = server.arg("TZ1"); // Selected Time Zone
    NTPSN = server.arg("NTPS"); NTPSN.trim(); // NTPServer-Name
    if (NTPSN != "") {ntpServerName = NTPSN.c_str();} else {ntpServerName = "ca.pool.ntp.org";}
...
int16_t n = WiFi.scanNetworks(false, true); //WiFi.scanNetworks(async, show_hidden)
    ...
    HTMLString += F("<br/>"); 
    HTMLString += F("<br/>");
    HTMLString += "<div class='form-floating'><label>NTP-Serverpool (Example: ca.pool.ntp.org)</label><input type='text' class='form-control' name='NTPS' id='NTPS' value='"+NTPSN+"'></div>";
    HTMLString += F("<br/>"); 
    // *********************************** Select Time Zone **************************************************************************************************************
