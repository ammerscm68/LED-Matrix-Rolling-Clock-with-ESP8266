...
// for ESP-01 module
// LedControl lc=LedControl(2,0,3,1);
...
if (littleFSInit == true)
{
  ...
  WiFiManagerParameter MatrixDisplayOn("MDOnTime", "Matrix Display On Time (Example: 7:30:0)",MDOnTime, 10);
  WiFiManagerParameter MatrixDisplayOff("MDOffTime", "Matrix Display Off Time (Example: 23:45:0)",MDOffTime, 10);
...
void loop()
{
  ...
  // Matrix einschalten
  if (Display == 1 && DisplayOnTime != "") {
  if  (String(h)+":"+String(m)+":"+String(s) == DisplayOnTime)
  {
  if (await == false) 
  { 
  await = true;   
  ...
  if  (String(h)+":"+String(m)+":"+String(s) == DisplayOffTime)
  {
  if (await == false) 
  {  
  await = true;  
