...
time_t getNtpTime() {
  ...
  while (millis() - beginWait < 1500) {
    ...
    if (size >= NTP_PACKET_SIZE) {
      WaitForNTPIP = 0;
      if (SerialMonitor) {Serial.println(F("Empfange Antwort vom NTP-Server"));}
  ...
  if (SerialMonitor) {Serial.println("NTP-Time - keine NTP Antwort - "+FormatedDateTimeDE(false));}
  // return 0; // gibt 0 zurück, wenn die Zeit nicht ermittelt werden kann.
  printStringWithShift("        +++        Der NTP-Server antwortet nicht !      +++           ",ScrollTextTime);
  // Neustart Uhr
  if (SerialMonitor) {Serial.println("Die Uhr wird neu gestartet - "+FormatedDateTimeDE(false));}
  printStringWithShift("        +++       Die Uhr wird neu gestartet      +++           ",ScrollTextTime);
  ESP.restart();
  return false;}
