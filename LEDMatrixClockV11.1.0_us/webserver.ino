...
OnlyReboot = true; ResetClockSettings(); OnlyReboot = false;}} // Just Restart instead of Reset 
...
if (littleFSInit && ResetNow) {ResetClock();}} else { // Settings to Standard - WLAN Access Data will be deleted 
HTMLAnswer("PW");}} 
...
String RefreshTime = F("60"); // Refresh Website after X seconds
  ...
  if (IntInTempValue >= 50 && IntInTempValue <= 83) {InTempOnOff1 = F("id= 'Gvalue'>Temperature on the Clock:");} else {  
...
// 1. Part of the HTML string
  ...
  HTMLString += F("h2 {color: white; font-size: 16px; margin-bottom: 10px;}");
  HTMLString += F("h3 {color: white; font-size: 16px; margin-bottom: 10px;}");
  HTMLString += F("h4 {font-size: 14px; color: #2ECCFA;}");
  HTMLString += F("h5 {color: white;font-size: 22px; margin-left: 1px; margin-bottom: 10px;}");
  HTMLString += F("h6 {font-size: 14px; color: #2ECCFA;}");
  HTMLString += F("h7 {font-size: 14px; color: #ffffff;}");
  HTMLString += F("h8 {font-size: 14px; color: #f74f4f; font-weight: bold;}");
  HTMLString += F("h9 {font-size: 18px; margin-left: 80px; color: #f79d36; font-weight: bold;}");
  HTMLString += F("h10 {font-size: 14px; color: #ffcc00;}");
...
if (!MatrixAction && Display) {HTMLString += F("<h11>[ Until the Display automatically switches off, only the Time will be shown! ]</h11>");}
HTMLString += F("<p></p>");
HTMLString += F("<hr style=\"border-top: 3px solid #095484;width: auto;\">");
return HTMLString;}  
...
String HTMLString; HTMLString.reserve(180);
// ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
// ESPRAM = ESP.getMaxFreeBlockSize() / 1024;  // Check RAM
...
HTMLString += "<h7>Free Memory: &nbsp;&nbsp;"+String(ESPRAM)+" Byte / 49152 Byte</h7>";} else {
HTMLString += "<h8>Free Memory: &nbsp;&nbsp;"+String(ESPRAM)+" Byte / 49152 Byte (Danger ! - too little)</h8>";}
HTMLString += F("<a><br /></a>");
HTMLString += "<h7>CPU Clock Frequency: &nbsp;&nbsp;"+String(CPUClock)+"&nbsp; MHz</h7>";
HTMLString += F("<a><br /></a>");
HTMLString += "<h7>Startdate: &nbsp;&nbsp;"+StartDate+"</h7>";
HTMLString += F("<a><br /></a>");
if (ClockStatus == "OK") {
HTMLString += F("<h7>Last Start Status: &nbsp;&nbsp;OK</h7>");} else {
HTMLString += "<h10>Last Start Status: &nbsp;&nbsp;"+ClockStatus+"</h10>";}
HTMLString += F("<a><br /></a>");
...
HTMLString += F("<a href =\"/\"><button class=\"button\" style=\"margin-right: 60px; height: 48px; width: 120px;color: navy;float: right; font-size: 18px;\">Refresh</button></a>");
HTMLString += F("<a><br /></a>");
HTMLString += F("<a><br /></a>");
...
if (WebFunction == "WeatherFunctionsON1") {HTMLAnswerString = F("OpenWeatherMap.org - Weather Data function has been switched ON");}
if (WebFunction == "WeatherFunctionsON2") {HTMLAnswerString = F("OpenWeatherMap.org - Weather Data function is already switched ON");}
if (WebFunction == "WeatherFunctionsOFF") {HTMLAnswerString = F("OpenWeatherMap.org - Weather Data function has been switched OFF");}
if (WebFunction == "NewsDataFunctionON1") {HTMLAnswerString = F("NewsData.io - Current News has been switched ON");}  
if (WebFunction == "NewsDataFunctionON2") {HTMLAnswerString = F("NewsData.io - Current News is already switched ON");}
if (WebFunction == "NewsDataFunctionOFF") {HTMLAnswerString = F("NewsData.io - Current News has been switched OFF");}
...
if (WebFunction == "EnhancedFunctionsON1") {HTMLAnswerString = F("The additional Functions have been activated<br />(Date, Birthdays, Holidays)");}
if (WebFunction == "EnhancedFunctionsON2") {HTMLAnswerString = F("The additional Functions are already switched ON<br />(Date, Birthdays, Holidays)");}
if (WebFunction == "EnhancedFunctionsOFF") {HTMLAnswerString = F("The additional Functions have been switched OFF<br />(Date, Birthdays, Holidays)");}
...
HTMLString += F("<h4>Please wait ...</h4>");
if (Display) {HTMLString += F("<h3>(Until the time is shown again in the Matrix Display)</h3>");}
...
await = true;
HTMLAnswer("MatrixOn");
if (SerialMonitor) {DEBUG_PRINTLN(F("The LED Matrix was switched on."));}   
...
if (SerialMonitor) {DEBUG_PRINTLN(F("The LED Matrix is turned off."));} 
if (WatchOwner1FirstName != "") { 
SText = Politeness(true);} else {SText = Politeness(false);}
printStringWithShift(SText.c_str(),ScrollTextTime);
printStringWithShift("           ",ScrollTextTime);  
...
if (SerialMonitor) {DEBUG_PRINTLN(F("The Doublepoint of the Clock flashes."));}
} else {HTMLAnswer("PW");}}
...
if (SerialMonitor) {DEBUG_PRINTLN(F("The Doublepoint of the Clock is permanently ON"));}
} else {HTMLAnswer("PW");}}
...
if (SerialMonitor) {DEBUG_PRINTLN(F("The Doublepoint of the Clock is permanently OFF"));}
} else {HTMLAnswer("PW");}}
...
if (SerialMonitor) {DEBUG_PRINTLN("Brightness Level "+String(BLInt)+" enabled");}
} else {HTMLAnswer("PW");}}
...
if (SerialMonitor) {DEBUG_PRINTLN("Scrolling Text Speed Level "+String(STLevel)+" activated");}
...
if (SerialMonitor) {DEBUG_PRINTLN(F("OpenWeatherMap - Weather Data switched on"));}}
else {
HTMLAnswer("WeatherFunctionsON2");
if (SerialMonitor) {DEBUG_PRINTLN(F("OpenWeatherMap - Weather Data is already turned on"));}}   
} else {HTMLAnswer("PW");}}
...
if (!await) {   
// DEBUG_PRINTLN(F("Additional Functions switched off"));
...
if (SerialMonitor) {DEBUG_PRINTLN(F("OpenWeatherMap - Weather Data switched off"));}
} else {HTMLAnswer("PW");}}
...
if (SerialMonitor) {
DEBUG_PRINTLN(F("NewsData - Current News has been switched ON"));
if (AccessGMCMap && AccessNewsData) {DEBUG_PRINTLN(F("+++   The Clock will restart   +++"));}}
HTMLAnswer("NewsDataFunctionON1");} else {
HTMLAnswer("NewsDataFunctionON2");
if (SerialMonitor) {DEBUG_PRINTLN(F("NewsData - Current News is already switched ON"));}} 
} else {HTMLAnswer("PW");}}
...
if (SerialMonitor) {DEBUG_PRINTLN(F("NewsData - Current News is switched off"));}
} else {HTMLAnswer("PW");}}
...
if (SerialMonitor) {
DEBUG_PRINTLN(F("GMCMap - Current Radioactivity Levels were Switched ON"));
if (AccessGMCMap && AccessNewsData) {DEBUG_PRINTLN(F("+++   The Clock will restart   +++"));}}
HTMLAnswer("GMCMapFunctionON1");} else {
HTMLAnswer("GMCMapFunctionON2");
if (SerialMonitor) {DEBUG_PRINTLN(F("GMCMap - Current Radioactivity Levels are already Switched ON"));}} 
} else {HTMLAnswer("PW");}}
...
if (SerialMonitor) {DEBUG_PRINTLN(F("GMCMap - Current Display of Radioactivity Switched off"));}
} else {HTMLAnswer("PW");}}
...
if (SerialMonitor) {DEBUG_PRINTLN(F("Additional Functions switched on - (Date, Birthdays, Holidays)"));} 
} else {HTMLAnswer("PW");}}
...
if (SerialMonitor) {DEBUG_PRINTLN(F("Additional Functions switched off - (Date, Birthdays, Holidays)"));}
} else {HTMLAnswer("PW");}}
...
HTMLString += "<h2>Time Zone:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"+MCTZ+"</h2>";
HTMLString += F("<a><br /></a>");
...
HTMLString += "<h2>GMCMap.com --- GMT-Offset:&nbsp;&nbsp;"+String(utco)+"</h2>";
HTMLString += F("<a><br /></a>");}
if (IsPushover) {
...
HTMLString += F("<h2>Pushover-Devicename2:  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;n/a</h2>");}  
if (PushoverDevice3 != "") {
HTMLString += "<h2>Pushover-Devicename3:  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"+PushoverDevice3+"</h2>";} else {
HTMLString += F("<h2>Pushover-Devicename3:  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;n/a</h2>");}  
HTMLString += "<h2>Pushover-Messagesound:  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"+PushoverMessageSound+"</h2>";
HTMLString += F("<a><br /></a>");}
if (MidnightGhost == "YES") {HTMLString += F("<h2>Show Ghosts at Midnight ?:  &nbspYES</h2>");} else {HTMLString += F("<h2>Show Ghosts at Midnight ?:  &nbspNO</h2>");}
HTMLString += F("<a><br /></a>");
if (InTempOffset == "999") {HTMLString += F("<h2>Temperature and Humidity Sensor ?:  &nbspdisabled</h2>");} else {
if ((InTempValue != "!" && InHumValue == "-") || (InTempValue == "!" && InHumValue != "-")) {HTMLString += F("<h2>Temperature and Humidity Sensor ?:  &nbspNO</h2>");} else {
if (DHTSensor == "DHT11") {HTMLString += F("<h2>Temperature and Humidity Sensor ?:  &nbspDHT11</h2>");} else {HTMLString += F("<h2>Temperature and Humidity Sensor ?:  &nbspDHT22</h2>");}}}
HTMLString += F("<a><br /></a>");
if (IsBuzzer) {
...
InHumidityStatus = CheckInHumidity(); // Measuring Indoor Air Humidity
HTMLString.clear();
if (SerialMonitor) {DEBUG_PRINTLN(F("Settings from the Access Point (Captive Portal) are called up"));}}
else {HTMLAnswer("PW");}}
...
HTMLString += F("<a><br /></a>");
if (OnlyReboot) {
HTMLString += F("<h3>Insufficient RAM !</h3>");
HTMLString += F("<h2>(Restarting the Clock resolves the Problem.)</h2>");
HTMLString += F("<a><br /></a>");
HTMLString += F("<h2>Restart LED-Matrix-Clock now ?</h2>");
HTMLString += F("<a><br /></a>");
HTMLString += F("<a href =\"/reboot\"><button class=\"button\" style=\"height: 48px; width: 110px; font-size: 22px; color: red\">YES</button></a>");  
} else {
HTMLString += F("<h3>LED-Matrix-Clock Reset !</h3>");
HTMLString += F("<h2>(The WLAN Access Data will be deleted)</h2>");
HTMLString += F("<a><br /></a>");
HTMLString += F("<h2>Now Reset the LED Matrix Clock ?</h2>");
HTMLString += F("<a><br /></a>");
HTMLString += F("<a href =\"/goreset\"><button class=\"button\" style=\"height: 48px; width: 110px; font-size: 22px; color: red\">YES</button></a>");}
HTMLString += F("<a href =\"/\"><button class=\"button\" style=\"margin-left: 80px;height: 48px; width: 110px; font-size: 22px;\">NO</button></a>");
HTMLString += F("</body>");
HTMLString += F("</html>");  
OneTimeCall = true; 
SendHTMLString(HTMLString); // Send HTMLString
...
HTMLString = F("<!DOCTYPE html> <html lang='en-US'>\
  ...
  <style>\
   ...
   h3 {font-size: 14px; margin-top: 0px; margin-bottom: 50px;}\
    ...
    <h3>(Restarting the Clock might fix the Problem)</h3>\
...
if (server.method() == HTTP_POST) {
	...
	// If MidnightGhost then switches off one Minute further at 00:00
    ...
    if (SerialMonitor) {DEBUG_PRINTLN(F("Display automatically ON/OFF Data saved... "));} 
    ...
    RebootNow = true;
     ...
     if (SerialMonitor) {DEBUG_PRINTLN(F("Access Data and Settings were not saved !"));}
    HTMLString = HTMLStringPart1();
    HTMLString += F("line-height:1.5;border-radius:.3rem;width:100%}.form-signin{width:100%;max-width:420px;padding:15px;margin:auto;}h1,p{text-align: center}"); 
    HTMLString += F("h1 {font-size: 42px; color: #2ECCFA; margin-top: 50px; margin-bottom: 0px;}");   
    HTMLString += F("h5 {font-size: 16px; color: #f5f5f5; text-align:center; margin-top: 0px; margin-bottom: 0px;}"); 
    ...
    MyWaitLoop(100);}} else {HTMLAnswer("PW");}}  // Send HTMLString or wait
...
if (SerialMonitor) {DEBUG_PRINTLN(F("Buzzer Startsound switched on"));}
...
if (SerialMonitor) {DEBUG_PRINTLN(F("Buzzer Startsound switched off"));}
...
await = false;
if (SerialMonitor) {DEBUG_PRINTLN(F("Buzzer Hoursound switched on"));}} else {HTMLAnswer("PW");}
...
if (SerialMonitor) {DEBUG_PRINTLN(F("Buzzer Hoursound switched off"));}
