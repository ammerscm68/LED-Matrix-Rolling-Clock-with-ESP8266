void DHTHTML() { 
  ...
  if (SerialMonitor) {DEBUG_PRINTLN("Indoor Temperature: "+InTempValue+" °F");}
...
if (DHTSensor != "NoDHT") { 
  ...
  ITempValue = String(dht.toFahrenheit(TempValue),0); // Output in Fahrenheit
  ...
  if (SerialMonitor) {DEBUG_PRINTLN("Indoor Temperature: "+ITempValue+" °F");} // only Debugmodus
...
String InHumMessure() {
  ...
  LastInTempMessureDateTime = FormatedDateTimeUS(true);}} else {IHumValue = F("-");} 
  return IHumValue;}
...
MyWaitLoop(100); // Short Wait
  ...
  InHumValue = F("-");
  RLF = F("FAIL");
  InHumMessage.clear();}} else {StatusWarningLED("LOW"); RLF = F("FAIL");}} else {RLF = F("NoDHT");} // LED off and Fail or NoDHT
  return RLF;}
