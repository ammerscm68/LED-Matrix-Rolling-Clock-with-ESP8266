...
String authors[authors_Count] = {F("The Washington Post"), F("Yahoo! News"),  // Json "source_name" US Authors
                                 F("Chronicle Online"), F("Norfolk Daily News"), F("Americanbankingnews"), F("News-daily"),  
                                 F("Nbc 5 Dallas"), F("News-gazette"), F("Newspressnow"), F("Gazette"), F("Dailygazette.com"),
                                 F("Upi"), F("Newsitem"), F("The Business Journals"), F("Cbs News"), F("Daily Journal"),
                                 F("Daily Item"), F("Thedailynewsonline"), F("Lincoln Journal Star"), F("Idahostatejournal")};  
...
while (!client.connected()) {ConnectCount += 1; MyWaitLoop(100); if (ConnectCount > 45) {break;}} // short Wait
...
HTTPClient http;  
   ...
   while (!http.connected()) {ConnectCount += 1;MyWaitLoop(100);if (ConnectCount > 45) {break;}} // short Wait
    ...
    // Data Search
    if (SerialMonitor) {DEBUG_PRINTLN("Parse Authors: "+FormatedDateTimeUS(false));}
