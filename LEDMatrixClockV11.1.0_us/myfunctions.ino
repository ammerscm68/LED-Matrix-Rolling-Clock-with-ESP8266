...
GT.clear(); HT.clear();                      // deg = ° Grad Sign
String GreetingText;                         // WatchOwner Only
int16_t HDay = 0;                            // uss = ß Sign  
...
if (HolidayDate == "14.2") {HoliDayText = F("Valentine's Day");} // Valentine's Day (Not a Holiday - Special Exception)
if (HolidayDate == Third_Monday(1)) {HoliDayText = F("Martin Luther King Day");}   // Martin Luther King Day
if (HolidayDate == Third_Monday(2)) {HoliDayText = F("Presidents’ Day");}   // Washington’s Birthday
if (HolidayDate == Last_Monday(5)) {HoliDayText = F("Memorial Day");}   // Memorial Day
if (HolidayDate == "19.6") {HoliDayText = F("Juneteenth National Independence Day");} // Juneteenth National Independence Day
if (HolidayDate == "4.7") {HoliDayText = F("Independence Day");}   // Independence Day
if (HolidayDate == First_Monday(9)) {HoliDayText = F("Labor Day");}   // Labor Day
if (HolidayDate == Second_Monday(10)) {HoliDayText = F("Columbus Day");}   // Columbus Day
if (HolidayDate == "11.11") {HoliDayText = F("Veterans' Day");}   // Veterans' Day
if (HolidayDate == FourthThursday(11)) {HoliDayText = F("Thanksgiving");}   // Thanksgiving
...
if (SerialMonitor) {
DEBUG_PRINTLN(F(""));   
DEBUG_PRINTLN("Holiday: "+HT1); // View Holiday Text
DEBUG_PRINTLN(F(""));}} else { // New Year's Eve
HT= F("       +++     We wish you a good Start into the new Year     +++                ");
HT1= F("+++ We wish you a good Start into the new Year +++");
if (SerialMonitor) {
DEBUG_PRINTLN(F(""));   
DEBUG_PRINTLN("ToDay: New Year's Eve"); // View New Year's Eve Text
DEBUG_PRINTLN(F(""));}}
if (Display && !GHStart) { // Output HoliDayText when Display on
if (HolidayDate != "1.1") {  
printStringWithShift(HT.c_str(), ScrollTextTime);
if (HolidayDate == "14.2") {ValentinesDay();}} else {  // Valentine's Day Display of Hearts 
printStringWithShift("           ", ScrollTextTime); 
for (byte i=0; i < 3; i++){ // 3x New Year's Greetings    
printStringWithShift("          +++     Happy New Year     +++                ", ScrollTextTime);}
ClockScrollIn(); SIC = false;}}} else {
if (SerialMonitor) {
DEBUG_PRINTLN(F(""));   
DEBUG_PRINTLN(F("Holiday: No Holiday"));
DEBUG_PRINTLN(F(""));}}} // End of the Function "void Greetings_and_Holiday()"
...
void ShowGhost() {
if (SerialMonitor) {DEBUG_PRINTLN(F("Ghost-Hour ....."));} 
if (Display) {printStringWithShift3("         ",ScrollTextTime+5);
Ghost();} else {printStringWithShift3("         ",ScrollTextTime+5);  
sendCmdAll(CMD_SHUTDOWN,1); Ghost(); // Turn on Matrix and display Ghost
sendCmdAll(CMD_SHUTDOWN,0);}} // Switch off Matrix

// *************************************************************************************************

void Ghost() {
String GShow; 
// Special Characters  (Font: clockfont[])
...
if (SerialMonitor) {DEBUG_PRINTLN(F("Load Clock Adjustments ..."));}
  ...
  if (NTPSN != "") {ntpServerName = NTPSN.c_str();} else {ntpServerName = "us.pool.ntp.org";} // Default
 ...
 DEBUG_PRINTLN(F(""));}
  ...
  LoadClockStatus(); // Load the last Status of the Clock
    ...
    MCTZ = F("New York, Detroit, Miami, Lima"); // Default Time Zone
    ntpServerName = "us.pool.ntp.org";  // Find local Server under http://www.pool.ntp.org/zone
    }
...
void SaveMatrixAdjustments(){
  ...
  // No longer needed and will be deleted if present
...
if (SerialMonitor) {DEBUG_PRINTLN("Matrix switched off via Button at "+ FormatedDateTimeUS(false));}
APStart += 1; // inc   
} else { 
    MyWaitLoop(500); // wait     
    getTimeLocal(); // Get current Time 
    sendCmdAll(CMD_SHUTDOWN, 1); // Display On
    InHumidityStatus = CheckInHumidity(); // Measuring Indoor Humidity
    if (IsBuzzer) {ButtonDisplayOnBeep();}
    if (SerialMonitor) {DEBUG_PRINTLN("Matrix switched on via Button at "+ FormatedDateTimeUS(false));}
    APStart += 1;}} // inc 
...
String FormatedDateTimeUS(bool FDTHTML) {
String DateTimeString;
getTimeLocal(); // get curennt DateTime
// Time
...
if (d < 10){DateTimeString += "/0"+String(d)+"/"+String(ye);} else {DateTimeString += "/"+String(d)+"/"+String(ye);}
...
if (SerialMonitor) {DEBUG_PRINTLN("UDP Server started - "+FormatedDateTimeUS(false));}}

// *************************************************************************************************

void WiFiUDPStop() {
Udp.stop();
if (SerialMonitor) {DEBUG_PRINTLN("UDP Server stopped - "+FormatedDateTimeUS(false));}}

// *************************************************************************************************

void WebServerStart() {
server.begin();
if (SerialMonitor) {DEBUG_PRINTLN("Web Server started - "+FormatedDateTimeUS(false));}}

// *************************************************************************************************

void WebServerStop() {
server.stop();
if (SerialMonitor) {DEBUG_PRINTLN("Web Server stopped - "+FormatedDateTimeUS(false));}}
...
long WiFiRSSI() {
const int8_t N = 20; // Number of Measurements for the Mean
...
rssiIndex = (rssiIndex + 1) % N;  // Ring buffer logic
if (!bufferfull) bufferfull = (rssiIndex == 0);
// 3. Calculate average
count = bufferfull ? N : rssiIndex;  // How many values ​​are already valid ?
for (byte i = 0; i < count; i++) {
sum += rssiBuffer[i];} MyWaitLoop(5);} // short wait
...
int8_t WifiRSSI = WiFiRSSI(); // WiFi Signal Strength in dBm
...
bool ByteRAM = false;
if (Modul == "GMC" || Modul == "Gmc" || Modul == "gmc") {MinRAM = 16312;} // GMC requires at least 16312 bytes of RAM.
if (SerialMonitor) {
DEBUG_PRINTLN(F(""));
