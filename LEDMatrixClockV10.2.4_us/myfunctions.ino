...
if (SerialMonitor) {Serial.println("Matrix switched off via Button at "+ FormatedDateTimeUS(false));} 
    ...
    // ShortBuzzerBeep();
    if (SerialMonitor) {Serial.println("Matrix switched on via Button at "+ FormatedDateTimeUS(false));}} else
    {
...
WiFi.begin(ssid, password);
while (WiFi.status() != WL_CONNECTED) { 
if (Start && !await) {MyWaitLoop(1000);} else {MyWaitLoop(250);}
if (SerialMonitor) {Serial.print(".");} 
if (!Start && Display && !await && StartStopServers > 0) {FlashNoWLANDots(1);}
WiFiConnectLoop += 1;
if (WiFiConnectLoop >= MaxWiFiConnectLoop) {break;}}
