...
HTTPClient http;  
OWMLastConnectionDateTime = FormatedDateTimeDE(); // Connection Time;
...
Serial.println(F("Wetterdaten werden heruntergeladen - Bitte warten ..."));}
   ...
   MyWaitLoop(1000); //wait
    ...
    // Daten auswerten
    DynamicJsonDocument doc(1024); // für ESP8266 
 // DynamicJsonDocument doc(ESP.getMaxFreeBlockSize() - 1024); // für ESP8266 
 // DynamicJsonDocument doc(ESP.getMaxAllocHeap()); // für ESP32 
    ...
    if (SerialMonitor) {
    Serial.println("Wetterdaten von: "+FormatedDateTimeDE());
