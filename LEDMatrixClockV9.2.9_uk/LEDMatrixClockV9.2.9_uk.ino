...
// ############################################################################################################################################# 
int MinComfortHum = 32; // Minimum Comfort Humidity (default = 32) 
int MaxComfortHum = 68; // Maximum comfort Humidity (default = 68)
// #############################################################################################################################################
...
String HT =  ""; // Holliday
String HT1 = ""; // Holliday
int BDCounter = 0; // Counter for Greetings/Holidays and News
...
// #####################################################################################
  ...
  if (!IsBuzzer) {
  if (SerialMonitor) {Serial.println("Buzzer not active");}}
...
// contrast(); //AutoBrightness
 ...
 if (!await) {MatrixButtonOnOff();} 
  ...
  dots = 1; // Doublepoint permanently lit
  ...
  WebServerStart();}} // Start web server
...
// ***************************************************************************************************************************
Greetings_and_Holiday(); // First, greetings if available 
sendCmdAll(CMD_SHUTDOWN,0); // Display off
...
await = false;} // Start of the clock - End
  ...
  if (MidnightGhost) {ShowGhost();} // Ghost at Midnight
  getTimeLocal(); 
  if (EnhancedFunctions) {Greetings_and_Holiday();} // Determine Greetings and Holidays for the new Day 
  ...
  if  (String(m)+":"+String(s) == DateNewsViewTime[i]+":8") { 
    ...
    DateString = "          +++         Today is " + dayName[w] + "   " + FormatedDay + "." + FormatedMonth + "." + String(ye)+ "        +++               ";
    SIC = true; // ScrollIn Clock
    ...
    if (BDCounter == 0) {Greetings_and_Holiday();} else {GreetingsToday = false;}
    BDCounter++;
    if (BDCounter > 1) {BDCounter = 0;}} // Alternately display Greetings and Holidays as well as News
    // ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
