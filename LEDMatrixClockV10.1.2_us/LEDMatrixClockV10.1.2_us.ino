...
String GMCMMAD = "30";  // Example: 30 --> So no Download of the Current GMCMap Data until 00:30 o'clock
// #############################################################################################################################################
...
String rtime = ""; // Runtime of the Clock
...
// NTP Server Pool:
static const char ntpServerName[] = "pool.ntp.org";    //Find local Server under http://www.pool.ntp.org/zone
...
// #####################################################################################
  ...
  if (SerialMonitor) {   
  Serial.println("StartTime: "+FormatedDateTimeUS(false));
  Serial.println(F(""));
  Serial.println(F(""));}
  ...
  if (DHT11Sensor) { 
  if (SerialMonitor) {Serial.println("DHT11 Sensor is Connected. ("+InTempValue+"°F)");}} else {
  if (SerialMonitor) {Serial.println("DHT22 Sensor is Connected. ("+InTempValue+"°F)");}}  
...
void loop() {  
 ...
 if (!await) {MatrixButtonOnOff();} 
  ...
  if (SerialMonitor) {Serial.println("WiFi Connection: disconnected - "+FormatedDateTimeUS(false));
  ...
  if (SerialMonitor) {Serial.println("WiFi Connection restored - "+FormatedDateTimeUS(false));
...
await = false;} // Start of the clock - End
  ...
  SText = Politeness(true);} else {SText = Politeness(false);}
  printStringWithShift(SText.c_str(),ScrollTextTime);
  StatusWarningLED("LOW"); // LED off then on 
  ...
  printStringWithShift("        +++       Attention: The Humidity at the Location of the Watch is too low... !      +++           ",ScrollTextTime);
  ...
  // Humidity too high
  for (int i=0; i <= 1; i++){ // Repeat Message twice
  printStringWithShift("        +++       Attention: The Humidity at the Location of the Watch is too high... !      +++           ",ScrollTextTime);
  MyWaitLoop(1500);}} 
  if (InHumidityStatus != "MIN" && InHumidityStatus != "MAX") {StatusWarningLED("LOW");} // LED Aus
