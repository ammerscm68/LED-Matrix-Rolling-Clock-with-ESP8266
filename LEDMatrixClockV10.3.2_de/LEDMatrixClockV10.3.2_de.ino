 ...
 ********************************************************************************************
*/
const char* LMCVersion = "10.3.2 de";
...
bool DHT11Sensor = false; // true = DHT11  /  false = DHT22   Sensor    (Ausgabe der Temperatur in Celsius) - Ab Version 10.2.9 im Captiv Portal änderbar
// ##########################################################################################################################################################
...
// ##########################################################################################################################################################
int MaxWiFiConnectLoop = 20;  // Maximale Anzahl Loops bei Verbindung mit dem WLAN  -  Minimum = 15
// ##########################################################################################################################################################
...
String DateNewsViewTime[] = {"9","18","27","42","54"}; // aktuelles Datum, Greetings und News (Minute)
...
if (SerialMonitor) {
InitSerial(115200, 120); DEBUG_PRINTLN("");} //Warten auf COM Verbindung  /  Warten Default= 120
...
await = false;} // Start der Uhr - Ende
  ...
  DateString = ExpandedDateDE();} // Datum mit Kalenderwoche berechnen
  if (WeatherFunctions && AccessOpenWeatherMap) {getWeatherData();} // Neuen Sonnenauf- und Sonnenuntergang ermitteln
  ...
  if (IsPushover) {MEP=true; PushOverAlert(InHumidityStatus); MEP=false;}  // Nachricht über Pushover senden  
