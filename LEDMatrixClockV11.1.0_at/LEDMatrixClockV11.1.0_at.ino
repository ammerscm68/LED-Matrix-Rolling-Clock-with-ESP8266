 ...
 ********************************************************************************************
*/
const char* LMCVersion = "11.1.0 at";
...
String DHTSensor = "NoDHT"; // DHT11 / DHT22 / NoDHT   Sensor    (Ausgabe der Temperatur in Celsius)
// ##########################################################################################################################################################
...
await = false;} // Start der Uhr - Ende
  ...
  if  (String(m)+":"+String(s) == OnlyTempViewTime[i]+":11") {    
    ...
    // *** Außentemperatur ***
    if (WeatherFunctions && AccessOpenWeatherMap){
    if (IsEnoughRAM(MinRAMMem,"Wetter") && IsWiFiSignalOK()) {  // nur wenn genügend RAM zur Verfügung steht  
