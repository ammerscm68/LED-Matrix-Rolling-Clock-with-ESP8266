void DHTHTML() { 
  ...
  if (SerialMonitor) {DEBUG_PRINTLN("Innentemperatur: "+InTempValue+" °C");}
  MyWaitLoop(500); // kurz warten
  if (InTempOffset == "999") {InHumValue = "0";} else {InHumValue = InHumMessure();} // Luftfeuchte Wert abrufen
  if (SerialMonitor) {DEBUG_PRINTLN("Innenluftfeuchte: "+InHumValue+" %");}
...
if (DHTSensor != "NoDHT") {  
  ...
  DEBUG_PRINTLN(F("Innentemperatur wird gemessen ..."));}
  ...
  if (ITempValue == "!") {
  if (SerialMonitor) {DEBUG_PRINTLN(F("keinen gültigen Wert vom Sensor empfangen !"));}
  ...
  if (SerialMonitor) {DEBUG_PRINTLN("Innentemperatur: "+ITempValue+" °C");} // nur im Debugmodus
...
// ====================================================================================================================================
  ...
  if (SerialMonitor) {DEBUG_PRINTLN(F(""));
  DEBUG_PRINTLN(F("Luftfeuchte wird gemessen ..."));}
  ...
  if (SerialMonitor) {DEBUG_PRINTLN("Innenluftfeuchte: "+IHumValue+" %");} // nur im Debugmodus
  LastInTempMessureDateTime = FormatedDateTimeDE(true);}} else {IHumValue = F("-");} 
  return IHumValue;}
...
MyWaitLoop(100); // kurz warten
  ...
  InHumValue = F("-");
  RLF = F("FAIL");
  InHumMessage.clear();}} else {StatusWarningLED("LOW"); RLF = F("FAIL");}} else {RLF = F("NoDHT");} // LED Ausschalten und Fail oder NoDHT
  return RLF;}
...
if (server.method() == HTTP_POST) {
    ...
    if (SerialMonitor) {DEBUG_PRINTLN(F("DHT-Sensor Offset Daten gespeichert... "));}
    // Website anzeigen  
    HTMLString = HTMLStringPart1();
    HTMLString += F("line-height:1.5;border-radius:.3rem;width:100%}.form-signin{width:100%;max-width:420px;padding:15px;margin:auto;}h1,p{text-align: center}");
    HTMLString += F("h1 {font-size: 42px; color: #2ECCFA; margin-top: 50px; margin-bottom: 0px;}");
    ...
    // DHT-Sensor Offset Einstellungen wurden nicht gespeichert
     if (SerialMonitor) {DEBUG_PRINTLN(F("DHT-Sensor Offset Einstellungen wurden nicht gespeichert !"));}
    HTMLString = HTMLStringPart1();
    HTMLString += F("line-height:1.5;border-radius:.3rem;width:100%}.form-signin{width:100%;max-width:420px;padding:15px;margin:auto;}h1,p{text-align: center}"); 
    HTMLString += F("h1 {font-size: 42px; color: #2ECCFA; margin-top: 50px; margin-bottom: 0px;}");   
    HTMLString += F("h5 {font-size: 18px; color: #f5f5f5; text-align:center; margin-top: 0px; margin-bottom: 0px;}"); 
