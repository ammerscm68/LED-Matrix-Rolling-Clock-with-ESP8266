...
// #####################################################################################
  ...
  if (SerialMonitor) {   
  Serial.println("StartTime: "+FormatedDateTimeUS());
  Serial.println(F(""));
  Serial.println(F(""));}
...
// contrast(); //AutoBrightness
 ...
 if (!await) {MatrixButtonOnOff();} 
  ...
  if (SerialMonitor) {Serial.println("Please wait ... "+FormatedDateTimeUS());}
  ...
  StartStopServers -= 1;
  if (SerialMonitor) {Serial.println("WiFi Connection: restored - "+FormatedDateTimeUS());}
...
printStringWithShift("           ",1);
sendCmdAll(CMD_SHUTDOWN,1); // Display switch on
ClockScrollIn(); // Scroll the time in the Display
await = false;} // Start of the clock - End
  ...
  if  (String(m)+":"+String(s) == OnlyHumViewTime[i]+":8") { 
    ...
    SIC = true; // ScrollIn Clock
    printStringWithShift(InHumString.c_str(), ScrollTextTime); // Indoor Humidity measurement
