...
// #####################################################################################
  ...
  if (SerialMonitor) {   
  Serial.println("Startzeit: "+FormatedDateTimeDE());
  Serial.println(F(""));
  Serial.println(F(""));}
...
// contrast(); //AutoBrightness
 ...
 if (!await) {MatrixButtonOnOff();} 
  ...
  if (SerialMonitor) {Serial.println("Bitte warten ... "+FormatedDateTimeDE());}
  ...
  StartStopServers -= 1;
  if (SerialMonitor) {Serial.println("WLAN-Verbindung: wieder hergestellt - "+FormatedDateTimeDE());}
...
// nur Innen- und Außenluftfeuchte --- 3x pro Stunde (default)
  ...
  if  (String(m)+":"+String(s) == OnlyHumViewTime[i]+":8") { 
    ...
    SIC = true; // ScrollIn Clock
    printStringWithShift(InHumString.c_str(), ScrollTextTime); // Innenluftfeuchte anzeigen
    MyWaitLoop(10000); // kurz warten (10 sek. - default)
