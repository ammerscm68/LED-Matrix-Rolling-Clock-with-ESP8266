...
if (SerialMonitor) {Serial.println(F("Lade Einstellungen der Uhr ..."));}
  ...
  NAF.trim();
  if (NAF == "" || NAF == "On") {NewsAPIFunction = true; StatusNewsAPI = F("aktiviert");}
  if (NAF == "Off") {NewsAPIFunction = false; StatusNewsAPI = F("deaktiviert");}} else {
  NewsAPIFunction = true; StatusNewsAPI = F("aktiviert");}} else {
  NewsAPIFunction = false; StatusNewsAPI = F("deaktiviert");}  
...
void SaveMatrixAdjustments(){
  ...
  if (!IsNumeric(InHumOffset) ||  InHumOffset == "") {InHumOffset = F("0");} // Humidity
...
if (SerialMonitor) {Serial.println("Matrix per Taster um "+ FormatedDateTimeDE() + " ausgeschaltet.");} 
}
else
    {
    ...
    // ShortBuzzerBeep();
    if (SerialMonitor) {Serial.println("Matrix per Taster um "+ FormatedDateTimeDE() + " eingeschaltet.");}} else
    {
...
if (SerialMonitor) {Serial.println("UDP Server gestartet - "+FormatedDateTimeDE());}}

// *************************************************************************************************

void WiFiUDPStop() {
Udp.stop();
if (SerialMonitor) {Serial.println("UDP Server gestoppt - "+FormatedDateTimeDE());}}

// *************************************************************************************************

void WebServerStart() {
server.begin();
if (SerialMonitor) {Serial.println("Webserver gestartet - "+FormatedDateTimeDE());}}

// *************************************************************************************************

void WebServerStop() {
server.stop();
if (SerialMonitor) {Serial.println("Webserver gestoppt - "+FormatedDateTimeDE());}}
