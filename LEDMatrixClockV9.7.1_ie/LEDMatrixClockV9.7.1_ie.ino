...
// #####################################################################################
  ...
  if (SerialMonitor) {   
  Serial.println("StartTime: "+FormatedDateTimeEN());
  Serial.println(F(""));
  Serial.println(F(""));}
...
// contrast(); //AutoBrightness
 ...
 if (!await) {MatrixButtonOnOff();} 
  ...
  if (SerialMonitor) {Serial.println("Please wait ... "+FormatedDateTimeEN());}
  ...
  StartStopServers -= 1;
  if (SerialMonitor) {Serial.println("WiFi Connection: restored - "+FormatedDateTimeEN());}
