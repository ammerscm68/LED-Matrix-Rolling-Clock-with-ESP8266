...
// #####################################################################################
  ...
  if (ssid == "") {
    Serial.println(F(""));
    Serial.println(F("***** First Start Clock *****")); // erster Start der Uhr
    Serial.println(F(""));
    }}
  ...
  Serial.println(F(""));  
  Serial.println(F("DHT-Sensor ist defekt !"));} else {Serial.println(F("kein DHT-Sensor angeschlossen"));}
  Serial.println(F(""));}}  
...
// nur Innen- und Außenluftfeuchte --- 3x pro Stunde (default)
  ...
  TriggerHumityLED(); // Luftfeuchte LED Ein- oder Ausschalten wenn Sensor vorhanden und OK
    ...
    if (SerialMonitor) {Serial.println(F("OpenWeatherMap wurde deaktiviert - keine Wetterdaten"));} 
