...
String DisplayOnTime, DisplayOffTime, DisplayTimeChange, FDOnTime, FDOffTime;
...
void setup()
{
  ...
  if (ssid != "") {greatings();} // Als erstes Greetings 
...
{
if (ssid == "") {
if (WebServerWait == false) {server.handleClient();}
} else {
 ...
 if (await == false) { 
  ...
  // ***** Matrix einschalten ******
  if (Display == 0 && DisplayOnTime != "" && DisplayOnTime != "---") {
  ...
  // ****** Matrix abschalten *******
  if (Display == 1 && DisplayOffTime != "" && DisplayOffTime != "---") {
  ...
  await = true;  
  Display = 0;  
  if (h >= 0 && h < 12) {printStringWithShift("        +++  Guten Morgen ...   schoenen Tag noch  +++           ",ScrollTextTime);} 
  if (h >= 12 && h < 18) {printStringWithShift("        +++  schoenen Tag noch  +++           ",ScrollTextTime);} 
  if (h >= 18 && h < 21) {printStringWithShift("        +++  Guten Abend ...   bis bald +++           ",ScrollTextTime);} 
