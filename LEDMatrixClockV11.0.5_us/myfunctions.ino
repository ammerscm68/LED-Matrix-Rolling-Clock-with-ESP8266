...
if (SerialMonitor) {DEBUG_PRINTLN(F("Load Clock Adjustments ..."));}
 ...
 // Selection DHT-Sensor [DHT11 or DHT22]
 ...
 if (SerialMonitor) {
 DEBUG_PRINTLN(F(""));  
 DEBUG_PRINTLN(F("DHT-Sensor Selection : [DHT11 / DHT22] ")+ DHTSensor);
 DEBUG_PRINTLN(F(""));}
  ...
  GMCMapFunction = false; StatusGMCMap = F("disabled");} 
    ...
    // Select News or GMC (unfortunately both are not possible because the memory is not large enough)
