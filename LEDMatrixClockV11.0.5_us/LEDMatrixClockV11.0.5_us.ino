 ...
 *******************************************************************************************
*/
const char* LMCVersion = "11.0.5 us";
...
bool DHT11Sensor = false; // true = DHT11  /  false = DHT22   Sensor    (Output the Temperature in Celsius)
// #############################################################################################################################################
...
// #############################################################################################################################################
int16_t WGPMinRAMMem = 12288; // Minimum RAM in Byte  Default = 12288  (For Weather, GMC, Pushover)
int16_t NMinRAMMem   = 14024; // Minimum RAM in Byte  Default = 14024  (For News)
// #############################################################################################################################################
...
// #############################################################################################################################################
String MCTZ = "New York, Detroit, Miami, Lima"; // Presetting the Time Zone in which the Clock is located (Changes possible in the Captiv Portal)
// #############################################################################################################################################

// #############################################################################################################################################
String NTPSN = "us.pool.ntp.org";
const char* ntpServerName = "us.pool.ntp.org";  // Finde lokale Server unter http://www.pool.ntp.org/zone
// #############################################################################################################################################
...
// Website Display shows insufficient RAM
...
// #####################################################################################
  ...
  if (SerialMonitor) {   
  DEBUG_PRINTLN("StartTime: "+FormatedDateTimeUS(false));
  DEBUG_PRINTLN(F(""));
  DEBUG_PRINTLN(F(""));}
  ...
  if (DHT11Sensor) { 
  if (SerialMonitor) {DEBUG_PRINTLN("DHT11 Sensor is Connected. ("+InTempValue+"°F)");}} else {
  if (SerialMonitor) {DEBUG_PRINTLN("DHT22 Sensor is Connected. ("+InTempValue+"°F)");}}  
  ...
  DateString = ExpandedDateUS(); // Calculate Date and Calendar Week
...
void loop() {  
 ...
 if (!await) {MatrixButtonOnOff(false);} 
  ...
  SaveClockStatus("WiFi Connection: disconnected -/- "+FormatedDateTimeUS(false)); // Save Status of the Clock
  StartStopServers += 1;  
  WebServerStop(); // Stop web Server
  WiFiUDPStop(); // Stop UDP Server
  StatusWarningLED("HIGH");} // LED ON
  if (SerialMonitor) {DEBUG_PRINTLN("WiFi Connection: disconnected - "+FormatedDateTimeUS(false));
  ...
  if (SerialMonitor) {DEBUG_PRINTLN("WiFi Connection Restored - "+FormatedDateTimeUS(false));
...
if (IsEnoughRAM(WGPMinRAMMem,"GMC")) {  // Only if enough RAM is available 
...
GMCMapMessage(5,2);} else {freeRam();}} // GMCMap Evaluate and display Data
...
await = false;} // Start of the clock - End
  ...
  DateString = ExpandedDateUS();}
  if (WeatherFunctions && AccessOpenWeatherMap && IsEnoughRAM(WGPMinRAMMem,"Weather")) {MEP=true; getWeatherData(); MEP=false;} // Determine new Sunrise and Sunset Times
  ...
  MEP=true; getGMCMapData(); MEP=false; // Get current Radioactivity Values
  printStringWithShift("             ", ScrollTextTime); // Run GMCMap from the Display
  GMCMapMessage(5,2);} else {freeRam();}} // Evaluate and display GMCMap data or defragment RAM
  ClockScrollIn();  // Scroll the time in the Display
  ...
  printStringWithShift("             ", ScrollTextTime); // Run "GMCMap" from the Display
  GMCMapMessage(10,5);} else {freeRam();}}  // Evaluate and display GMCMap data or defragment RAM
  ...
  if  (String(m)+":"+String(s) == OnlyTempViewTime[i]+":11") {    
    ...
    if (IsEnoughRAM(WGPMinRAMMem,"Weather")) {  // Only if enough RAM is available   
    ...
    } else {freeRam();}}  // or defragment RAM
  ...
  if  (String(m)+":"+String(s) == DateNewsViewTime[i]+":11") { 
    ...
    DateString = ExpandedDateUS(); // Calculate date with calendar week
    SIC = true; // ScrollIn Clock
    printStringWithShift("        ", ScrollTextTime);
    ...
    InHumidityStatus = CheckInHumidity(); // Measuring Indoor Air Humidity
    if (IsEnoughRAM(NMinRAMMem,"News")) {  // Only if enough RAM is available 
    if (APINews != "") {
    SIC = true; // ScrollIn Clock  
    printStringWithShift("        ", ScrollTextTime);
    printStringWithShift(APINews.c_str(), ScrollTextTime);}} else {freeRam();}} // Display News or defragment RAM
    // ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    if (SIC) {ClockScrollIn(); SIC = false;} // ScrollIn Clock 
  ...
  printStringWithShift("        ", ScrollTextTime);
  printStringWithShift(weatherString.c_str(), ScrollTextTime);} else {freeRam();} // Scroll into the display to see the time or defragment the RAM.
  if (SIC) {ClockScrollIn(); SIC = false;} // ScrollIn Clock
  PDP = false; // Colon permanently off again when Flashing Mode
  await = false;}}}
  ...
  if (SIC) {ClockScrollIn(); SIC = false;} // Scroll the Time in the Display
  PDP = false; // Colon permanently off again when Flashing Mode
  await = false;}}}}
  }  // OnMatrixAction() = True
