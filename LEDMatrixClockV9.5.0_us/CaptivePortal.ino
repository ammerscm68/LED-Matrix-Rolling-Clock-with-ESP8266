...
if (server.method() == HTTP_POST) {
    ...
    if (littleFSInit){
    HTMLString = "<!doctype html><html lang='en-US'>";
    ...
    HTMLString += "<br/>";
    HTMLString += "<h6>Access Data and Settings have been Saved<br />The Clock will Restart</h6>";
    HTMLString += "</main>";
    HTMLString += "</body>";
    HTMLString += "</html>";
    ...
    if (SerialMonitor) {Serial.println("Access Data Saved... ");} 
    printStringWithShift("        +++       Access Data and Settings saved...      +++           ",ScrollTextTime); 
    if (SerialMonitor) {Serial.println("The Clock will Restart");} 
    printStringWithShift("        +++       The Clock will Restart      +++           ",ScrollTextTime);
    // Reset
    ESP.restart();} else
     { // Access data was not saved
     if (SerialMonitor) {Serial.println("Access Data was not saved !");}
     HTMLString = "<!doctype html><html lang='en-US'>";
    HTMLString += "<head><meta charset='utf-8'>";
    HTMLString += "<meta name='viewport' content='width=device-width, initial-scale=1'>";
    HTMLString += "<title>Captive-Portal</title>";
    ...
    HTMLString += "<h5>("+ProgramName+")</h5>";
    HTMLString += "<br/>";
    HTMLString += "<h6>Error saving Settings<br />The Clock will Restart</h6>";
    HTMLString += "</main>";
    HTMLString += "</body>";
    HTMLString += "</html>";
    SendHTMLString(HTMLString); // Send HTMLString 
    ...
    HTMLString += "<h5>("+ProgramName+")</h5>";
    HTMLString += "<br/>";
    HTMLString += "<h6>The Settings are incomplete or invalid !<br />Settings were not Saved</h6>";
    HTMLString += "<br/>";
    HTMLString += "<a href =\"/\"><button class=\"button\">back to Homepage</button></a>";
    HTMLString += "</main>";
    HTMLString += "</body>";
    HTMLString += "</html>";
    SendHTMLString(HTMLString);} // Send HTMLString 
...
int n = WiFi.scanNetworks(false, false); //WiFi.scanNetworks(async, show_hidden)
    ...
    HTMLString += "<br/>";  
    HTMLString += "<br/>"; 
    HTMLString += "<h6>No WiFi found !</h6>";
    HTMLString += "<br/>";}
    HTMLString += "<br/>";
    ...
    if (InTempValue != "-") {
    if (DHT11Sensor) { 
    HTMLString += "<div class='form-floating'><br/><label>DHT11-Sensor Temp. Offset (Example: -1 or +1)</label><input type='text' maxlength='3' class='form-control' name='DHTTo' id='DHTTo' value='"+InTempOffset+"'></div>";
    ...
    HTMLString += "<div class='form-floating'><br/><label>DHT22-Sensor Temp. Offset (Example: -1 or +1)</label><input type='text' maxlength='3' class='form-control' name='DHTTo' id='DHTTo' value='"+InTempOffset+"'></div>";
    ...
    if (IsBuzzer) {
    HTMLString += "<div class='form-floating'><br/><label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Use a passive piezo Buzzer&nbsp;";
    HTMLString += "(Example: KY-006)</label><input type='checkbox' class='form-control' name='IsBuZZer' id='IsBuZZer' value='True' checked></div>";} else {
    HTMLString += "<div class='form-floating'><br/><label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Use a passive piezo Buzzer&nbsp;";
    HTMLString += "(Example: KY-006)</label><input type='checkbox' class='form-control' name='IsBuZZer' id='IsBuZZer' value='True'></div>";}
    // ******************************************* Parameter ************************************************************************************************************************************  
    return HTMLString;}  
...
String CPRootHTMLString_3(){
    ...
    HTMLString += "<button type='submit'>Refresh</button>";
    HTMLString += "</form>";
    HTMLString += "</main>";
    HTMLString += "</body>";
    HTMLString += "</html>";
...
HTMLString += "<h5>("+ProgramName+")</h5>";
HTMLString += "<br/>";
HTMLString += "<h6>The Site was not found !</h6>";
HTMLString += "<br/><br/>";
HTMLString += "<a href =\"/\"><button class=\"button\">back to Homepage</button></a>";
HTMLString += "</main>";
HTMLString += "</body>";
