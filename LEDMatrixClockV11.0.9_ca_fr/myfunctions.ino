...
if (SerialMonitor) {DEBUG_PRINTLN(F("Charger les Ajustements de l’Horloge ..."));}
 ...
 DEBUG_PRINTLN(F(""));}
  ...
  GMCMapGMTOffset.trim();} else {GMCMapGMTOffset.clear();} // www.gmcmap.com Décalage GMT
...
int8_t WifiRSSI = WiFi.RSSI(); // Intensité du Signal WiFi en dBm
...
// *************************************************************************************************
  ...
  InHumidityStatus = CheckInHumidity(); // Mesurer l'humidité intérieure
  ...
  if (InHumidityStatus == "MIN") {SIC = true; // Horloge de défilement    
  for (byte i=0; i <= 1; i++) { // Répétez le message deux fois
  printStringWithShift(cleanText("        +++       Attention : L'Humidité à l'emplacement de l'Horloge est trop faible... !      +++           ").c_str(),ScrollTextTime);
  MyWaitLoop(1500);}}
  if (InHumidityStatus == "MAX") {SIC = true; // Horloge de défilement   
  // Humidité trop élevée
  for (byte i=0; i <= 1; i++){ // Répétez le message deux fois
  printStringWithShift(cleanText("        +++       Attention : L'Humidité à l'emplacement de l'Horloge est trop élevée... !      +++           ").c_str(),ScrollTextTime);
  MyWaitLoop(1500);}}
  if (InHumidityStatus != "MIN" && InHumidityStatus != "MAX") {SIC = true; // Horloge de défilement  
  printStringWithShift(cleanText("        +++       L'Humidité à l'emplacement de l'Horloge est bonne...      +++           ").c_str(),ScrollTextTime);}}}
