...
if (server.method() == HTTP_POST) {
    ...
    if (littleFSInit){
    HTMLString = "<!doctype html><html lang='de-AT'>";
    ...
    HTMLString += "h3 { font-size: 16px; color: red; text-align:center; margin-top: 0px; margin-bottom: 15px;}";
    HTMLString += "h4 { font-size: 18px; color: red; text-align:center; margin-top: 0px; margin-bottom: 15px;}";
    ...
    HTMLString += "<br/>";
    HTMLString += "<br/>";
    HTMLString += "<h4>Achtung:</h4>";
    HTMLString += "<h3>Es werden nur die bundesweiten Feiertage angezeigt.</h3>";}
    HTMLString += "</main>";
    HTMLString += "</body>";
    HTMLString += "</html>";
...
int n = WiFi.scanNetworks(false, false); //WiFi.scanNetworks(async, show_hidden)
    ...
    HTMLString += "<br/>"; 
    // HTMLString += "<select name='' style=\"margin-right: 10px; margin-left: 3px; height: 42px; width: 150px; font-size: 14px; color: blue\">";
    // ********* Bundesländer einlesen *********
    if (IsFederalStateOK(FederalState)) {
    HTMLString += "<div class='form-floating'><br/><label>Wählen Sie hier Ihr Bundesland in Östereich aus:</label>";   
    HTMLString += "<select name='WBl' id='WBl' class='form-control'>";
