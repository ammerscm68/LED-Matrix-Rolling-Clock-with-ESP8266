...
MyWaitLoop(100); // kurz warten
  ...
  digitalWrite(HWLEDPin, LOW); // LED Ausschalten
  InHumMessage = F("");}
  } else {RLF = F("NORMAL"); InHumMessage = F("DHT-Sensor deaktiviert !"); digitalWrite(HWLEDPin, LOW);} // LED Ausschalten
  } else {
  digitalWrite(HWLEDPin, LOW); // LED Ausschalten
  InHumValue = F("!");
  RLF = F("FAIL");
  InHumMessage = F("");}} else {digitalWrite(HWLEDPin, LOW); RLF = F("FAIL");} // LED Ausschalten und Fail
  return RLF;  
}
