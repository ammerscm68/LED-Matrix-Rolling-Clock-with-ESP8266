...
// ##### Angabe des österreichischen Bundeslandes für die Feiertagsberechnung (siehe ganz unten)  - kann ab Version 9.4.0 im Captiv Portal geändert werden ###
...
// NTP Serverpool für Österreich:
static const char ntpServerName[] = "at.pool.ntp.org";    //Finde lokale Server unter http://www.pool.ntp.org/zone/at
...
// #####################################################################################
  ...
  if (!NewsAPIFunction) {
  Serial.println(F(""));
  Serial.println(F("NewsAPI wurde deaktiviert - keine aktuellen Nachrichten"));
  Serial.println(F("")); 
   } else {Serial.println(F(""));Serial.println(F(""));}}
...
if (InTempOffset != "999") {digitalWrite(HWLEDPin, HIGH);} // LED Einschalten 
MyWaitLoop(100);
...
await = false;} // Start der Uhr - Ende
  ...
  if (String(h) == "0" && String(m) == "0" && String(s) == "5" && Display) {PDP = true;} // Doppelpunkt an      
  ...
  if  (String(h)+":"+String(m)+":"+String(s) == DisplayOffTime+":7") {PDP = true;} // Doppelpunkt an     
  if  (String(h)+":"+String(m)+":"+String(s) == DisplayOffTime+":8") {     
  await = true;
  Display = false;  
  if (IsBuzzer && (StartSound == "On" || HourSound == "On")) {beep(NOTE_F4, 800); MyWaitLoop(1000);} // Matrix Abschalten Beep
  if (WatchOwner1FirstName != "") { 
  SText = Politeness(true);} else {SText = Politeness(false);}
  printStringWithShift(SText.c_str(),ScrollTextTime);
  digitalWrite(HWLEDPin, LOW); // LED Aus wenn Eingeschaltet
  ...
  if (IsBuzzer && (StartSound == "On" || HourSound == "On")) {beep(NOTE_F4, 800);} // Matrix Einschalten Beep
  ...
  if  (String(m)+":"+String(s) == OnlyTempViewTime[i]+":11") {    
    ...
    digitalWrite(HWLEDPin, LOW); // LED Ausschalten
    InTempValue = F("-");}} else {digitalWrite(HWLEDPin, LOW);} // LED Ausschalten
...
// nur Innen- und Außenluftfeuchte --- 3x pro Stunde (default)
  ...
  if  (String(m)+":"+String(s) == DateNewsViewTime[i]+":11") { 
    await = true;
    CheckInHumidity(); // Innenluftfeuchte Messen
    if (NewsAPIFunction && AccessNewsAPI) {getNews();} // aktuelle News
