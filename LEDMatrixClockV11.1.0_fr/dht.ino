void DHTHTML() { 
  ...
  if (SerialMonitor) {DEBUG_PRINTLN("Humidité de l'air Intérieur: "+InHumValue+" %");}
...
if (DHTSensor != "NoDHT") {   
  ...
  DEBUG_PRINTLN(F("La Température Intérieure est Mesurée ..."));}
  ...
  if (ITempValue == "!") {
  if (SerialMonitor) {DEBUG_PRINTLN(F("Aucune Valeur valide reçue du Capteur !"));}
  ...
  if (SerialMonitor) {DEBUG_PRINTLN("Température Intérieure: "+ITempValue+" °C");} // only Debugmodus
...
String InHumMessure() {
  ...
  if (SerialMonitor) {DEBUG_PRINTLN(F(""));
  DEBUG_PRINTLN(F("L'Humidité de l'Air Intérieur est Mesurée ..."));}
  ...
  if (SerialMonitor) {DEBUG_PRINTLN("Humidité de l'Air Intérieur: "+IHumValue+" %");} // only Debugmodus
  LastInTempMessureDateTime = FormatedDateTimeFR(true);}} else {IHumValue = F("-");} 
  return IHumValue;}
...
MyWaitLoop(100); // attends brièvement
  ...
  InHumValue = F("-");
  RLF = F("FAIL");
  InHumMessage.clear();}} else {StatusWarningLED("LOW"); RLF = F("FAIL");}} else {RLF = F("NoDHT");} // LED éteinte et échec ou absence de DHT
  return RLF;}
...
// ====================================================================================================================================
  ...
  if (IsPushover && (InHumidityStatus == "MIN" || InHumidityStatus == "MAX")) {PushOverAlert(InHumidityStatus);}  
  // Humidité trop faible
...
if (server.method() == HTTP_POST) {
    ...
    if (SerialMonitor) {DEBUG_PRINTLN(F("Données de décalage du Capteur DHT enregistrées..."));}
    ...
    // DHT Sensor Offset Settings were not saved
     if (SerialMonitor) {DEBUG_PRINTLN(F("Les Paramètres de décalage du Capteur DHT n'ont pas été Enregistrés !"));}
    HTMLString = F("<!doctype html><html lang='fr'>");
    HTMLString += F("<head><meta charset='utf-8'>");
    HTMLString += F("<meta name='viewport' content='width=device-width, initial-scale=1'>");
    HTMLString += F("<title>Horloge-Matricielle-LED</title>");
