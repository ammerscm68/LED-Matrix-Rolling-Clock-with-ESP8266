...
HTMLString += F("<a><br /></a>");
if (ESPRAM > 11) {
HTMLString += "<h7>Freier Arbeitsspeicher: &nbsp;&nbsp;"+String(ESPRAM)+" kB / 64 kB</h7>";} else {
HTMLString += "<h8>Freier Arbeitsspeicher: &nbsp;&nbsp;"+String(ESPRAM)+" kB / 64 kB (Achtung ! - zu wenig)</h8>";}
HTMLString += F("<a><br /></a>");
...
if (WebFunction == "NewsDataFunctionEin1") {HTMLAnswerString = F("NewsData.io - aktuelle Nachrichten wurden eingeschaltet<br />Wenn Sie auf die Startseite zurückkehren<br />wird die LED Matrix Uhr neu gestartet");}} else {
...
if (AccessGMCMap && AccessNewsData) {
if (WebFunction == "GMCMapFunctionEin1") {HTMLAnswerString = F("GMCMap.com - aktuelle Radioaktivitätswerte wurden eingeschaltet<br />Wenn Sie auf die Startseite zurückkehren<br />wird die LED Matrix Uhr neu gestartet");}} else {
...
HTMLAnswer("MatrixOn");
MEP = true; 
if (SerialMonitor) {Serial.println(F("Das LED-Matrix Display wurde eingeschaltet"));} 
...
if (NewsDataFunction) {StatusNewsData = F("aktiviert");}   
HTMLAnswer("MatrixOff");
if (SerialMonitor) {Serial.println(F("Das LED-Matrix Display wurde ausgeschaltet"));}  
if (WatchOwner1FirstName != "") { 
SText = Politeness(true);} else {SText = Politeness(false);}
printStringWithShift(SText.c_str(),ScrollTextTime);
printStringWithShift("           ",ScrollTextTime);  
...
// ++++++++++++++++++++++++++++++++++++++++++++++   
if (AccessGMCMap && AccessNewsData) {    
GMCMapFunction = false; // GMC deaktivieren 
...
if (SerialMonitor) {
Serial.println(F("NewsData - aktuelle Nachrichten eingeschaltet"));
if (AccessGMCMap && AccessNewsData) {Serial.println(F("+++   Die Uhr wird neu gestartet   +++"));}}
HTMLAnswer("NewsDataFunctionEin1");} else {
HTMLAnswer("NewsDataFunctionEin2");
if (SerialMonitor) {Serial.println(F("NewsData - aktuelle Nachrichten sind bereits eingeschaltet"));}} 
} else {PleaseWait();}}
...
if (SerialMonitor) {Serial.println(F("NewsData - aktuelle Nachrichten ausgeschaltet"));}
} else {PleaseWait();}}
...
// +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ 
if (AccessGMCMap && AccessNewsData) { 
NewsDataFunction = false;  // News deaktivieren wegen Speichermangel 
StatusNewsData = F("deaktiviert");
StatusWarningLED("LOW"); // LED Ausschalten
RebootNow = true;
if (littleFSInit) {
...
if (SerialMonitor) {
Serial.println(F("GMCMap - Anzeige der aktuellen Radioaktivität eingeschaltet"));
if (AccessGMCMap && AccessNewsData) {Serial.println(F("+++   Die Uhr wird neu gestartet   +++"));}}
HTMLAnswer("GMCMapFunctionEin1");} else {
HTMLAnswer("GMCMapFunctionEin2");
if (SerialMonitor) {Serial.println(F("GMCMap - Anzeige der aktuellen Radioaktivität ist bereits eingeschaltet"));}} 
} else {PleaseWait();}}
...
if (SerialMonitor) {Serial.println(F("GMCMap - aktuelle Anzeige der Radioaktivität ausgeschaltet"));}
} else {PleaseWait();}}
...
HTMLString += F("<a><br /></a>");}
if (MidnightGhost == "YES") {HTMLString += F("<h2>Geister um Mitternacht anzeigen ?:  &nbspJA</h2>");} else {HTMLString += F("<h2>Geister um Mitternacht anzeigen ?:  &nbspNEIN</h2>");}
HTMLString += F("<a><br /></a>");
...
await = false;
if (SerialMonitor) {Serial.println(F("Summer (Buzzer) Stundensound eingeschaltet"));}} else {PleaseWait();}
