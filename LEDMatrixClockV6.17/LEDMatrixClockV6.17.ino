...
// contrast(); //AutoBrightness
 ...
 if (await == false && TasterPin != -1) {MatrixButtonOnOff();} 
  ...
  if  (String(h)+":"+String(m)+":"+String(s) == DisplayOnTime+":2") {
  if (await == false){  
  await = true;    
  ...
  if  (String(h)+":"+String(m)+":"+String(s) == DisplayOffTime+":2"){
  if (await == false){  
  await = true;  
  Display = 0;  
  if (h >= 0 && h < 12)  {SText = "        +++  Guten Morgen ...   Sch"+uoe+"nen Tag noch  +++           ";} 
  ...
  if  (String(m)+":"+String(s) == OnlyTempViewTime[i]+":5") { 
    ...
    if (SIC == true) {ClockScrollIn(); SIC = false;} // Uhrzeit in Display hineinscrollen
    await = false;}}}
    
  // Zusatzfunktionen und/oder News Anzeigen --- alle 10 Minuten
  if (await == false){ 
  for (int i=0; i <= 4; i++){  
  if  (String(m)+":"+String(s) == DateNewsViewTime[i]+":5"){ 
    await = true;
    // ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  if (await == false && WeatherFunctions == true) { 
  for (int i=0; i <= 4; i++){  
...
if (WeatherFunctions == true || NewsAPIFunction == true) { 
...
getAnimClock(); // Uhrzeit anzeigen / berechnen 
