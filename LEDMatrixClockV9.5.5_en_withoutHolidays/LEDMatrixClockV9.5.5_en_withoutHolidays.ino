...
bool DHT11Sensor = true; // true = DHT11  /  false = DHT22   Sensor   - (Output Temperature in Celsius)
// #############################################################################################################################################
...
// NTP Server Pool for UK-England:
...
String HT =  ""; // Holliday
String HT1 = ""; // Holliday
int GNCounter = 1; // Counter for Greetings and News
...
// #####################################################################################
  ...
  if (!NewsAPIFunction) {
  Serial.println(F(""));
  Serial.println(F("NewsAPI has been disabled - No current News"));
  Serial.println(F("")); 
   } else {Serial.println(F(""));Serial.println(F(""));}}
  ...
  Serial.println(F("")); 
  if (DHT11Sensor) { 
  Serial.println("DHT11 Sensor is Connected. ("+InTempValue+"°C)");} else {
  Serial.println("DHT22 Sensor is Connected. ("+InTempValue+"°C)");}  
  Serial.println(F(""));} else {
  if ((InTempValue != "-" && InHumValue == "-") || (InTempValue == "-" && InHumValue != "-")) {  
  Serial.println(F(""));  
  Serial.println(F("DHT Sensor is malfunction !"));} else {Serial.println(F("No DHT Sensor connected"));}
  Serial.println(F(""));}}  
  ...
  DateString = ExpandedDate(); // Calculate Date and Calendar Week
...
await = false;} // Start of the clock - End
  ...
  if  (String(m)+":"+String(s) == OnlyHumViewTime[i]+":8") {     
    ...
    } else {if (SerialMonitor) {Serial.println(F("DHT Sensor deactivated or malfunction"));}
    digitalWrite(HWLEDPin, LOW); // LED turn off
    InTempValue = "-";}} else {digitalWrite(HWLEDPin, LOW);} // LED turn off
  ...
  TriggerHumityLED(); // Humidity LED Switch on or off if Sensor is Present and OK
    ...
    if (SerialMonitor) {Serial.println(F("OpenWeatherMap has been disabled - no Weather Data"));} 
