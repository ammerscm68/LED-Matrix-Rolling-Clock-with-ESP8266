...
if (server.method() == HTTP_POST) {
    ...
    DHTSensor = server.arg("DHT"); // Sélection du Capteur DHT [DHT11 ou DHT22]
...
int16_t n = WiFi.scanNetworks(false, true); //WiFi.scanNetworks(async, show_hidden)
    ...
    HTMLString += F("<br/>"); 
    HTMLString += F("<br/>");
    HTMLString += "<div class='form-floating'><label>Pool de Serveurs NTP (Exemple: ca.pool.ntp.org)</label><input type='text' class='form-control' name='NTPS' id='NTPS' value='"+NTPSN+"'></div>";
    HTMLString += F("<br/>"); 
    // *********************************** Select Time Zone **************************************************************************************************************
