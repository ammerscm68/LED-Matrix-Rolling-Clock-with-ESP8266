 ...
 *******************************************************************************************
*/
const char* LMCVersion = "11.0.5 ca-fr";
...
// #############################################################################################################################################
int16_t WGPMinRAMMem = 12288; // Minimale RAM in Byte  Default = 12288  (pour Wetter, GMC, Pushover)
int16_t NMinRAMMem   = 14024; // Minimale RAM in Byte  Default = 14024  (pour News)
// #############################################################################################################################################
...
String MCTZ = "Coral Harbour, Pond Inlet, Atikokan, Toronto, Montreal, Ottawa"; // 
// #############################################################################################################################################

// #############################################################################################################################################
String NTPSN = "ca.pool.ntp.org";
const char* ntpServerName = "ca.pool.ntp.org";  // Rechercher un serveur local sous http://www.pool.ntp.org/zone
// #############################################################################################################################################
...
// Sélection du Capteur DHT pour CaptivPortal
String DHTSensor;
...
// Check Minimale / Maximale "MaxWiFiConnectLoop"
...
if (IsEnoughRAM(WGPMinRAMMem,"GMC")) { // uniquement si suffisamment de RAM est disponible 
SIC = true; 
...
GMCMapMessage(5,2);} else {freeRam();}} // GMCMap Évaluer et afficher les données
...
await = false;} // Début de l'Horloge - Fin
  ...
  if (WeatherFunctions && AccessOpenWeatherMap) {getWeatherData();} // Determine new Sunrise and Sunset Times
  ...
  // Mesurer la Radioactivité
  if (GMCMapFunction && AccessGMCMap && GMCMapMidnightAccessDelay(GMCMMAD)) { 
  if (IsEnoughRAM(WGPMinRAMMem,"GMC")) { // seulement si suffisamment de RAM est disponible 
  ...
  printStringWithShift("             ", ScrollTextTime); // Exécutez « GMCMap » à partir de l'écran
  GMCMapMessage(10,5);} else {freeRam();}} // Évaluer et afficher les données GMCMap
...
//##############################################################################################################################################
  ...
  if  (String(m)+":"+String(s) == OnlyTempViewTime[i]+":11") {    
    ...
    if (IsEnoughRAM(WGPMinRAMMem,"Météo")) { // seulement si suffisamment de RAM est disponible 
    ...
    } else {freeRam();}} // ou défragmenter la RAM  
  ...
  if (NewsDataFunction && AccessNewsData) {MEP=true; getNews(); MEP=false;} // obtenir des nouvelles actuelles
    ...
    if (IsEnoughRAM(NMinRAMMem,"Actualités")) {  // seulement si suffisamment de RAM est disponible
    InHumidityStatus = CheckInHumidity(); // Mesure de l'humidité intérieure
    if (APINews != "") {
    SIC = true; // Horloge de défilement  
    printStringWithShift("        ", ScrollTextTime);
    printStringWithShift(APINews.c_str(), ScrollTextTime);}} else {freeRam();}} // Afficher les actualités
    // ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    if (SIC) {ClockScrollIn(); SIC = false;} // Horloge de défilement 
  ...
  printStringWithShift("          ", ScrollTextTime);
  printStringWithShift(weatherString.c_str(), ScrollTextTime);} else {freeRam();} // Faites défiler l'écran pour afficher l'heure ou défragmenter la RAM.
  ...
  MEP=true; getGMCMapData(); MEP=false; // Obtenir la valeur actuelle de la Radioactivité
  printStringWithShift("             ", ScrollTextTime); // Exécutez « GMCMap » à partir de l'écran
  GMCMapMessage(10,5);} else {freeRam();} // Évaluer et afficher les données GMCMap ou défragmenter la RAM
  if (SIC) {ClockScrollIn(); SIC = false;} // Faire défiler l'heure dans l'affichage
