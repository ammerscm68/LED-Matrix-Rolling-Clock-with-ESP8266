...
void printCharWithShiftR(unsigned char c, int shiftDelay) {
  if (c < ' ' || c > '~'+53) return;
  c -= 32;
  int w = showChar(c, font);
  for (int i=0; i<w+1; i++) {
    yield(); // Equivalent to  Application.ProcessMessages; 
    MyWaitLoop(shiftDelay);
    scrollRight(); // Scroll Right
    refreshAll();
    HandleOTAandWebServer();  // OTA and Webserver Quest
    yield(); // Equivalent to  Application.ProcessMessages; 
  }
}
...
void printCharWithShiftVO(unsigned char c, int shiftDelay) {
  if (c < ' ' || c > '~'+53) return;
  c -= 32;
  int w = showChar(c, font);
  for (int i=0; i<w; i++) {
