// Fonctions du MAX7219 par Pawel A. Hernik
// 2016.12.10 mis à jour pour les LED Martices pivotées, définissez ROTATE ci-dessous (0,90 ou 270)
...
byte scr[NUM_MAX*8 + 8]; // +8 pour les caractères défilés
...
void clr() {for (int i = 0; i < NUM_MAX*8; i++) scr[i] = 0;}

void scrollLeft() {for(int i=0; i < NUM_MAX*8+7; i++) scr[i] = scr[i+1];}

void invert() {for (int i = 0; i < NUM_MAX*8; i++) scr[i] = ~scr[i];}
...
sendCmdAll(CMD_INTENSITY, 0); // luminosité minimale
...
void contrast() {// luminosité automatique
  int lumina = analogRead(A0);         // luminosité automatique (+3.3V ---|==10k==|---A0---|GND (niq_ro)
    lumina = map(lumina, 0, 1023, 0, 4); // luminosité 0-4
