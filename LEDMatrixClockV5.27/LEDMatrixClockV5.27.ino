...
String weatherString, ATemp, DateString, InTempValue;
String ip;
...
String TempValue = ""; // Innentemperatur
...
void loop()
{
  ...
  { 
    ...
    // *** Innentemperatur ***
    if (InTempCounter < 5) {
    TempValue = TempMessure(); 
    if (TempValue != "-") {
    if (InTempCounter > 0) {InTempCounter = 0;} // Variable Reset
    String InTemp = "              Innentemperatur:        "+TempValue + deg + "C  "; 
    // Serial.println(InTemp); // Only for Debuging 
    printStringWithShift(InTemp.c_str(), ScrollTextTime); // Außentemperatur anzeigen 
    ...
    if (TempValue != "-" || StatusOpenWeatherMap == "OK") {ClockScrollIn();}} // Uhrzeit in Display hineinscrollen
    await = false;}}}
  ...
  getTimeLocal(); // aktuelle Uhrzeit holen
  for (int i=0; i <= 4; i++){  
  if  (String(m)+":"+String(s) == DateViewTime[i]+":5" || String(m)+":"+String(s) == DateViewTime[i]+":6"){ 
    await = true;
    ...
    greatings(); // Grüße
    if (!GreetingsToday && newsKey != "") {getNews();}  // aktuelle News
    // Anzeige News
...
for (int i=0; i <= 12; i++){  
if  (String(m)+":"+String(s) == DoublePointPermanantTime[i]+":4" || String(m)+":"+String(s) == DoublePointPermanantTime[i]+":5")
{
if (WeatherFunctions == true && AccessOpenWeatherMap == true)
{   
dots = 1; // Doppelpunkt Dauerleuchten  
