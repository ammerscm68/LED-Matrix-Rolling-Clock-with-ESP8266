...
char MDOnTime[6];
char MDOffTime[6];
char WMInTempOffset[4];
char WMInHumOffset[4];
...
void setup()
{
  ...
  // #######################################################
  // Start DNS Responder - call with "http://ledmatrixclock.local"
  Serial.println("DNS-Responder und Web-Server starten ...");
  if (!MDNS.begin("ledmatrixclock")) {
  Serial.println("Error setting up MDNS responder!");
  while (1) {MyWaitLoop(1000);}}
  Serial.println("mDNS Responder gestartet - URL: http://ledmatrixclock.local");
  ...
  Serial.println("[Loaded-DHT] InTempOffset: " + (String)WMInTempOffset);  // Only for Debuging
  Serial.println("[Loaded-DHT] InHumOffset: " + (String)WMInHumOffset);  // Only for Debuging*/ 
  ...
  printCharWithShift('.',ScrollTextTime);
    printCharWithShift('.',ScrollTextTime);
      printCharWithShift('.',ScrollTextTime);
  WiFiManagerParameter OpenWeatherMapApiKey("OWMapiKey", "OpenWeatherMap - API-Key (Optional)",OWMapiKey, 50);
  WiFiManagerParameter OpenWeatherCityID("OWMCityID", "OpenWeatherMap - City-ID",OWMCityID, 10);
  WiFiManagerParameter NewsAPIKey("NAPIKey", "NewsAPI - API-Key (Optional)",NAPIKey, 50);
  ...
  WiFiManagerParameter DHTTempOffset("WMInTempOffset", "DHT-Sensor Temperature Offset (Example: -1 or +1)",WMInTempOffset, 4);
  WiFiManagerParameter DHTHumOffset("WMInHumOffset", "DHT-Sensor Humidity Offset (Example: -1 or +1)",WMInHumOffset, 4);
  ...
  wifiManager.addParameter(&DHTTempOffset); // DHT-Sensor Temperature Offset
  wifiManager.addParameter(&DHTHumOffset); // DHT-Sensor Humidity Offset
  // ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  strcpy(WMInTempOffset,DHTTempOffset.getValue()); // DHT-Sensor Temperature Offset
  strcpy(WMInHumOffset,DHTHumOffset.getValue()); // DHT-Sensor Humidity Offset
  
  /*Serial.println("[Loop-WIFI] SSID: " + (String)wifiManager.getWiFiSSID());  // Only for Debuging 
  Serial.println("[Loop-WIFI] PASS: " + (String)wifiManager.getWiFiPass());   // Only for Debuging 
  Serial.println("[Loop-OpenWeatherMap] API-Key: " + (String)OWMapiKey);  // Only for Debuging 
  Serial.println("[Loop-OpenweatherMap] City-ID: " + cityID); // Only for Debuging 
  Serial.println("[Loop-NewsAPI] API-Key: " + newsKey); // Only for Debuging 
  Serial.println("[Loop-MatrixDisplay] OnTime: " + (String)MDOnTime);  // Only for Debuging  
  Serial.println("[Loop-MatrixDisplay] OffTime: " + (String)MDOffTime);  // Only for Debuging
  Serial.println("[Loaded-DHT] InTempOffset: " + (String)WMInTempOffset);  // Only for Debuging
  Serial.println("[Loaded-DHT] InHumOffset: " + (String)WMInHumOffset);  // Only for Debuging
  */ 
...
void loop()
{
  ...
  MDNS.update(); // DNS-Responder Update
  // Matrix einschalten
